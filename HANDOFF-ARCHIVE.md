# RAPTOR — handoff archive (frozen snapshot, 4 Sep 26)

**What this is.** A verbatim, frozen copy of `HANDOFF.md` as it stood on
4 Sep 26, just before it was cut down to a short current-state document.
Every resolved, closed and shipped session entry — the narrative of how each
thing was found, fixed and verified — is preserved here word for word. Nothing
was deleted in the cut; it was moved here.

**How to use it.** Do NOT read this file top to bottom at the start of a
session. Search it when a specific question needs history — "why did we decide
X", "what did the drag round try", "what was the gate reading in August" — and
read only the matching entry. `git log` is the authoritative record of what
changed; this is the story around it.

**Where the live truth is.** The current state (gates, in-flight work, open
items, standing constraints, deploy traps, file map) lives in `HANDOFF.md`.
The settled decisions live in `raptor-port/CLAUDE.md` §Stable decisions; the
contracts in `raptor-port/docs/` (engine-rules, ui-contracts, feature-impact,
performance). Anything below that is ALSO in one of those files is superseded
by that file — the snapshot below is history, not the live word.
