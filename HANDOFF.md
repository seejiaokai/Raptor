...
**Last recorded green baseline** (22 Aug 26, the member person-scope
tightening — the Inputs page's Person choice became a scheduler's, a member
files for the view-as person only; all six gates watched this session. It
sits on the same day's Inputs month calendar batch and the passes below):

| gate | reading |
|---|---|
| `npm test` | 2871 across 161 files — two vitest projects: raptor + leavewar |
...
Person, the member row editor, the write-path pins) → 2871; the two
audit relink tests now run their scheduler act under an admin session
(repaired in place, not added). The chain before that reconciled against
...
- **One owner decision still reserved off the Inputs month calendar (22 Aug
  26 — flagged in the ship report, not built).** A recurring input
  (`recur:'xN wks'`) chips only its FIRST span on the calendar, parity with
  `inputCoversDate` everywhere else — drawing every repetition is a real
  feature (the record stores one span), not a fix. Does not move without
  the owner. (The OTHER decision flagged beside it is CLOSED, 22 Aug 26 —
  owner: admin files inputs for anyone on both the page and the calendar; a
  member only for the view-as person. The Inputs page was tightened to
  match the calendar: the Person select is a scheduler's on the add form
  AND the row editor, a member sees the view-as callsign as a fixed value
  that follows the topbar live, and the write paths repeat the gate —
  `commitNewInput` pins to ME, `commitInputEdit` refuses a member person
  change. Rules: `docs/engine-rules.md` §Auth / roles; contract:
  `docs/ui-contracts.md` §The Inputs add form; pinned in
  `inputs.test.tsx`.)
...
- **THE DEMO IS SCRUBBED + HAS A SECOND, SWITCHABLE WEEK (owner, 21 Aug 26).**
  ...
  - **Per-week publish state is NOT persisted** — leaving a week and coming back
    starts it fresh (`resetSched` on every switch; the app is session-only
    anyway). The owner's end-state is persistent multi-week schedules you build
    ahead of time; that is the server/`HOOKS.storeBackend` work, and the
    `weekBundle` registry + the single `resetSched` seam are where it hooks in.
