...
the ground-crew sim-seat / NO_IR follow-up (13 Aug 26 — ground crew may now
sit either sim seat with no bar or warning, and a rear-seat passenger is no
longer rung by `NO_IR` on an IRT sortie; `avail.ts` `slotBar` and three
`validate.ts` guards, pinned by two new `personnel.test.ts` cases)**, run in
...
the ground-crew additions to the quals/html tests, +16; the sim-seat / NO_IR
follow-up +2), `node
...
| `src/engine/personnel.test.ts` | The ground-crew category (Aug 26) — that a personnel derives empty quals and the boot grants nothing, `slotBar` bars a JET front seat while allowing a rear jet seat, EITHER sim seat, duty and ground, validate raises `QUAL` in a jet front seat, the `PAX_CREW`/`CP` advisory in a rear jet seat, and `DOUBLE_BOOK`/`LONGDAY`/`DAYS_RUN` while crew rest, turns, the matrix, AAR, the sim seat and the brief/debrief windows stay OFF — including that a rear-seat passenger is never rung by `NO_IR`. Plus the parity guards (`PAX_CREW` has a WCODE, and fires nowhere on the seed). |
