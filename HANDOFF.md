...
| gate | reading |
|---|---|
| `npm test` | 3712 across 212 files (2 Sep 26, the OIL tracker batch: +20 `engine/oiltracker.test.ts`, +15 `ui/oiltracker.test.tsx`, +19 store/undo/counters pins) — two vitest projects: raptor + leavewar |
...
- **RESOLVED 2 Sep 26 (Leave War OIL TRACKER + admin-set LVE BAL + OFF USED
  ...
  row, the Logic page OIL row. Still open: the row drag is mouse/pen only (a
  finger scrolls; taps the boxes), no edge auto-scroll while dragging.
