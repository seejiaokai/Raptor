...
| `events.ts` | `collectEvents()` — the per-day event build the validator consumes; appends tomorrow's inputs shifted +1440 (the midnight tail, marked `nx`) and collects AVALON crew (`day.sacrew`, each entry with its `role`/`seat`) for the checks the wave's `noconf` does not cover. `scSeatHit`/`avSeatHit` (one `standaloneHit` body) — the model walk behind "one man in two SC / AVALON places in the same hours", seats AND the `sa`-marked desk, read by the validator and the crew picker alike (7 Sep 26). Also `inpShow(inp, dt)` — the per-day gate deciding whether an input reaches `day.input` (defer only on its accepted row's own day; SANS never); **exported** so the crew picker reads the SAME gate and cannot drift (Aug 26 audit). And `shiftEvHard`/`shiftHardGround` (26 Aug 26) — the SC MAIN clash grading with the ground-row red-list overlay (source type off `row.src` first, hand-typed keywords second), one body for the validator's clash loop and the picker's `live`. |
...
| `dutytpl.ts` | The squadron's **duty-block templates** (13 Aug 26) — mutable `DUTYTPL_CFG`, frozen `DUTYTPL_STD` (Standard / SC Shift / AVALON), `addTpl`/`delTpl`/`renameTpl`/`moveTpl`/**`setTplWave`** and the per-row `addTplRow`/`delTplRow`/`setTplRow`/`moveTplRow`, `blockFromTpl` (mints a `{label,rows}` duty block — PLAIN unless the template's `wave` names SC or AVALON, then `sa` + `noconf` for AVALON, 7 Sep 26), and `dutyTplSave`/`dutyTplLoad`/`dutyTplReset` against its own `dutytpl` key (an untrusted `wave` drops to none; a pre-7-Sep library keeps the seed desks' waves by id). Persisted state, exactly like stores; nothing in `validate.ts` reads a template. Loaded at boot in `initStore`. This is what `+ Block` offers now — waves no longer create desks (§Stable decisions). |
...
| `docs.ts` / `storage/docstore.ts` | **The supporting-document store (27 Aug 26; made durable 8 Sep 26)** — the home of the medical inputs' mandatory paperwork (`docAdd`/`docGet`/`docHas`, `DOC_MAX` 8 MB, images + PDFs). `docBackend.impl` is an in-memory cache (the SYNC read path the viewer needs in render); behind it a per-browser **IndexedDB** drawer (`raptor-docs`, `docstore.ts`), wired by `docBoot` from `main.tsx` on the BrowserBackend only (dev/tests/`?fresh` stay memory-only). `docAdd` writes through; `docBoot` fills the cache back at boot and advances the id counter past every stored id (a reset `seq` must not reuse a hydrated `doc<N>`); a corrupt row is skipped, a drawer that won't open falls back to memory-only. Deliberately NOT on `HOOKS.storeBackend` (a ~5 MB JSON seam a couple of photos would overflow is the wrong home for blobs); the drawer is the seam the future SHARED database replaces. APPEND-ONLY — undo can resurrect a deleted input and must find its document — and input records carry only the `docId` string, so history snapshots never copy a file. Object URLs are minted/revoked only by the viewer. Pinned in `docs.test.ts`. |
...
| `AvailWindow.tsx` | **The counter's window** ([ALL-AVAIL-WINDOW], 23 Sep 26, bug-checked FULL, holding for "merge live") — tapping an ALL/ALL AVAIL count chip on the board or the week opens one movable, resizable, NON-BLOCKING window of real pucks: pilots left, WSOs right, one per row, warning flags under each man; with OIL Earn on, a second tab switches men off earning (it REPLACED the mode's in-row crowd). State + position in `state/view.ts`, re-exported by `pops.ts` (outside React, so editing behind it never resets it; in state so the page/week/session resets can close it). It reads everything in the world its chip was drawn in (`html.ts withChipWorld`). The app's first third-kind transient surface — the outside-click rule does NOT apply; closes on ✕, page/week change, logout (D66). Contract: `docs/ui-contracts.md` §[ALL-AVAIL-WINDOW]. Pinned in `availwin.test.tsx`, `oilcount.test.tsx`, `oilrowpucks.test.tsx`, and `e2e/availwin.spec.ts` (paint, reach, placement). Evidence: `docs/handpass/2026-09-23-allavail-window.md`. |
...
| `DocConfirm.tsx` | **The medical-document ask (19 Sep 26, [SYNC-INTEG])** — the OilConfirm recipe (`.upconf-*` layer + `.docconf-pop`): saving a NEW medical (or one retyped INTO medical) with no certificate opens it BEFORE the write, from all three editors through the ONE gate `inputedit.tsx:docGate` (returns `'ask'`/`'ok'`). **Upload** dismisses (attach via the existing `DocField`, save again); **No document** resumes the SAME save with `skipDoc` so it flows on through the upchit-summary/downchit-clash/OIL pipeline and files with no doc. Replaced the old hard refusal; the replace-don't-strip guard stays at the write path. Every save/add/saveEdit binding passes a literal `false` (never React's event). Pinned in `docconfirm.test.tsx` (board dialog + the Inputs page add form) and `medwrite.test.ts` (docGate units). |
...
| `logout.ts` | THE ONE LOGOUT (23 Sep 26, D129): the top bar's and the phone drawer's Logout both call it; it asks the Tracker about unsaved chart edits (through `tracker/role.js`) before `resetSession`. |
...
| `ui/FigureCell.tsx` | **The two-line figure box (6 Sep 26)** — ONE component behind the real counter cell, the phone band's frozen copy and every drawer box, so the three cannot drift: `span.fb` (the balance, white; red with its minus below zero; a total always red) over `span.fu` (the days taken, no minus, `b.amber` LL / `b.red` the rest, absent when zero). Two 11px lines fill the existing 22px row, which is why rows do not grow when the drawer opens. Also owns the FLASH: the box fades from the accent tint once (700ms, `lw-figflash`) when THIS person's THIS figure changes — never on the first render, never when the column merely switches figures. Exports `show` (the rounding rule Matrix's own tooltip reads). |
| `ui/FiguresDrawer.tsx` | **The figures drawer (6 Sep 26)** — every figure for everyone, an absolute OVERLAY beside the names over the day columns (`div.mxdrawer[data-testid=figdrawer]`), built on the `.mxband` rules: drawn once, outside the sideways scroller, and never extra cells in the real table. Sideways coloured titles (`FigureTitle`, exported — the stuck bar's frozen copy draws the same ones as a plain span), boxes memoised per person (`PersonRow`'s lesson: eight figures × sixty people is 480 boxes, each reading that person's whole grid), rows keyed `data-drawer-key` so Matrix can copy the real rows' measured heights, and the title POP-UP (`.figpop`, screen-fixed, closing on an outside press / Escape / scroll / resize). Placed and sized entirely from numbers Matrix measures. Pinned: `figdrawer.test.tsx` + the drawer block in `e2e/leavewar.spec.ts` (both widths). |
| `ui/BalanceBar.tsx` | **The docked balance bar (6 Sep 26)** — what a drag down a figure column raises: `div.balbar` (`data-testid="balance-bar"`, `role="group"`), the credit form keyed on the FIGURE alone (a second drag that extends the run must not throw away a typed draft), the run passed as a prop, the amount opening EMPTY so an idle Enter adds nothing, autofocus on `(pointer: fine)` only. Deliberately NOT a `Sheet` (a Sheet's touch shield would swallow presses on the boxes being selected, and its popup family dismisses on scroll); it shares the move banner's dock recipe in `matrix.css` and keeps itself above a phone keyboard by following the visual viewport. Mounted in `Matrix.tsx` on admin + `selectableFigure` + a non-empty run. Pinned: `balancebar.test.tsx`, `figselect.test.tsx`, `leavewar.spec.ts`. Screen: `docs/ui-contracts.md` §Bulk balance entry from the figures. |
| `ui/CreditForm.tsx` | **The credit form and its `DayChip` (6 Sep 26)** — lifted out of `OilTracker.tsx` and generalised (a `+ / −` sign chip, a text amount box with `inputMode="decimal"` in place of the number spinner, Escape to cancel, and the last refusal cleared the moment the draft changes) so the tracker's bar and the grid's bar are ONE body and cannot drift about what a valid grant is. Generalised on `counter`: `reasonRequired` decides whether the date/reason/given-by half is drawn at all, `parseAmount(raw, sign)` is the shared amount rule (`2`/`+2` add, `-2` subtracts, a typed sign beats the `+ / −` chip), and the STORE (`grantTo`) still has the last word on non-zero and halves. |
| `ui/popat.ts` | Where a small screen-fixed pop-up goes beside the thing that opened it — clamped to the viewport, flipped above when it would fall off the bottom. Shared by the drawer's title pop-up and the quals popover so the two cannot place themselves differently. |
...
| `state/storage.ts` | The storage seam — `memoryBackend` and `localBackend` (the `leavewar:`-prefixed localStorage backend, still the store's default fallback for a store booted alone). Since the storage seam (8 Sep 26) `main.tsx` boots `lwInitStore` on the whiteboard-backed `leavewarAdapter` (`src/storage/adapters.ts`), so the WHOLE war persists per browser under `raptor:leavewar/*`, in lockstep with Raptor's `INPUTS`; before that it booted on `memoryBackend` (owner, 19 Aug 26: no persistence wanted then). Deliberately NOT `HOOKS.storeBackend`; the future shared database backend replaces the whiteboard's backend, not this seam. |
...
| `role.js` | The login-SESSION seam — a NO-IMPORT module so Raptor can reach it without loading the chart engine (`tracker.test.tsx` guards it): `resetSession` ends the Tracker's session through it, and every Logout (`src/ui/logout.ts`, the ONE logout both buttons call) first asks it about unsaved chart edits — Save / Discard / Stay (D129). The 7 Sep file lock is gone (D121: no roles in the Tracker). |
| `people.js` · `peoplewire.ts` · `people.test.ts` | The PEOPLE BRIDGE (9 Sep 26): `people.js` is a second no-import module (the roster list + `whoami`, signature-guarded setter, subscribe); `peoplewire.ts` is the Raptor side — projects `PEOPLE` (no archived/sentinel/ground) OCU-first into the bridge on every store notify and hands it `HOOKS.whoami()`; wired once by `TrackerPage.tsx`, imported by nothing under `app/` or `components/`. This is how `+ Add` in the Students card offers the squadron roster, and how marks get their `by`/`at` stamp. The link rides the roster entry's `pid` now (students, courses AND syllabuses are all stable ids since 1B-ii; the legacy `v3:links` block is read only for an old import). |
| `storage.js` | The storage doorway: async get/set/delete/list — through Raptor's whiteboard under `raptor:tracker/…` inside Raptor, bare `ocu:` localStorage only as the standalone fallback; `flushNow`/`loadLatest` no-ops. Replaced `sync/cloud.js` + `sync/local.js`. The database plugs in here. |
...
| `app/core.js` | The whole model + the imperatively rendered SVG flow board (3.1k lines, verbatim port). Adapted: imports `../storage.js` + `../role.js` (the login-session end only — no role: admin and member have the same access, File menu included, D121 23 Sep 26; the 7 Sep `fileLocked` gate is gone). The cloud button state and sinks are gone. **The file is a format, not a store (9 Sep 26):** the bound file handle, `fileDirty`, `saveToFileClick` and the menu's Charts/Students boxes are deleted; ✓ Save changes = `persistSyl()` only; `importClick` is ONE import for both jobs — charts chart by chart, then one "bring students & marks in too?" question when the file holds them (owner: "just have 1 button"). |
| `app/fileFormat.js` · `app/fileStore.js` · `app/eventOrder.js` | The saved file's shape (no browser APIs; **file version 3** — id-keyed charts/students + a `sylcat`, dual-shape reader, colon relaxed on chart/syllabus names); the picker wrapper (`pickOpen` reads on ANY browser — native picker on Chrome/Edge, a plain file input elsewhere, 9 Sep 26; Export writes in place on Chrome/Edge, downloads elsewhere; pickers need a live click); the Show All grouping/search — unchanged. |
| `app/eventDetails.js` · `eventDetails.test.tsx` | Event details PER CHART (D126): the pure diff/scrub/merge helpers `core.js` wires to `v3:master:eventinfo`, and the one-time carry-over of the old one-table `v3:eventinfo` (tested booting over it). |
| `retest.test.tsx` | Every defect the [HUMAN-RETEST] walk found (23 Sep 26), pinned red-first — F-, W1-, W2-, w3- and D12x-numbered after the evidence sheet `docs/handpass/2026-09-23-tracker.md`. |
| `app/ids.js` · `app/courseIds.js` · `app/sylIds.js` | The three PURE stable-id converters (mint/grammar/upgrade/reconcile), each shared by its once-per-browser migration in `core.js` and by file import so the two paths can't drift: **enrolment** ids (`ids.js`, 10 Sep), **course** ids (`courseIds.js`, 1B-i), **syllabus** ids (`sylIds.js`, 1B-ii — `BUILTIN_SYL` deterministic-id table + `classifyDefinedName`/`upgradeSyllabi`/`buildUnionSylcat`/`reconcileSylIds`). Tested by `*.test.ts` siblings + `sylIds.migration.test.ts` (the KEEP/RESET journal). |
| `components/` | Header (the same bar for every login, File menu included — D121; `#trUndoBtn`/`#trRedoBtn` — renamed from Raptor's ids — on the bar since 9 Sep 26), ArrangeTools, SidePanel, ShowAllPanel, Pop, Legend, Modals, ZoomControls — otherwise unchanged. |
...
| `src/engine/avalon-rules.test.ts` | AVALON's three seat rules + the SC desk (7 Sep 26, 45 tests) — SC NIGHT on a MAIN (not a SPARE; the seat, not the pilot), the pilots-only front seat MAIN and SPARE, one man in two AVALON places (MAIN+SPARE, seat+desk, desk+desk, the half-open abutting desk, `avSeatHit`'s selfKey), the picker saying the same words, the availability look re-pinned against a TEMPLATE-minted desk (ATT B mans it; HL/OML/ATT C/OL/OD flag; a plain desk would have DOUBLE_BOOKed his sortie), tomorrow's OD flagging tonight's MAIN, AVALON adding no OIL, the rot guard, and SC SPARE vs the SC desk (`scSeatHit`'s `what`, both pickers, the abutting PM desk, an ORDINARY desk still free). |
