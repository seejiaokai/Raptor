...
| `ArrangeSections.tsx` | The **Arrange sections** sheet (29 Aug 26) — a per-day sheet (`ARRANGESEC` in `pops.ts`) that re-orders the day's section panels (Programme · Flying waves · Duties · Sims · Ground Programme) with ▲▼ nudges + "Apply to all days". Opened by a `⇅ Arrange` button on both surfaces (board Templates bar via `boardMbtn`, edit-week `.dhtpl` via `interactions.ts`). Display order only — writes through `store.moveSection`/`applySecOrderToWeek` (histPush, no markEdit); the whole-day template remembers it. Contract: `docs/ui-contracts.md` §Arranging the schedule sections. |
