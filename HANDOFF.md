...
| `state/storage.ts` | The storage seam — `memoryBackend`, `localBackend` (the `leavewar:`-prefixed localStorage backend), and **`splitBackend` (19 Aug 26): `main.tsx` now boots `lwInitStore` on a split — the three manning-counter keys (`manningdefs`/`manningorder`/`manninghidden`) route to `localBackend` while everything else stays on `memoryBackend`, so the counters survive a reload and the war stays session-only like Raptor's `INPUTS`**. Deliberately NOT `HOOKS.storeBackend`; the future shared database backend replaces this seam. |
