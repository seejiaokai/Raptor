...
- **LEAVE WAR SPEED — PHASE 1 AND PHASE 2 SHIPPED (owner, 3 Sep 26 — "I'm
  ...
  **PHASE 2 — THE COLUMN WINDOW (`ui/colwindow.ts`, pure; the measuring and
  ...
  rows in idle slices or reuse the row DOM. (b) The remaining first-open JS
  ...
  drag). Desktop only. Contract: `docs/ui-contracts.md` §The column window;
  pin: e2e "a stable, year-wide scrubber that jumps the grid". This does NOT
  fix (a) — the jump the scrubber fires is the same far-jump rebuild, so a
  drag to a far month still costs a month jump.
