...
test:e2e` **258 passed / 6 touch-only skips** — re-measured on the merged
tree at the handoff, which is the only figure here anyone re-ran after the
last two commits landed; the +10 over wire 4's 248 is the admin-vs-member
medical row, the breakdown sheet, the viewer's lit row + "yours" picker +
member figures sheet, the scrolled sheet keeping its ✕, and the one-scroll
page lock, each at both LW viewports (the reorder and edit-person tests
were re-pointed at the gated/two-tap flows rather than added). **Two
interim counts in this file's history were wrong** — 254 was measured
before the last two commits added tests, and a commit message guessed 256;
restate a count only from a run you watched. Also re-run green on the
merged tree at the handoff: `npm test` 2475/137, `node reference/tfin.js`
728/0, `npm run build`,
`probes:adapted` 6/6 and `perf` 4/4 — **week 3743 / board 844, all ceilings
...
- **LEAVE WAR is merged as the sixth tab (16 Aug 26) and ALL FIVE SYNC WIRES
  ...
  **REFINED 18 Aug 26 — a REMARKS-ONLY edit no longer seizes the leave**
  ...
  refined remark (remarks are not in the signature). Ten tests in
