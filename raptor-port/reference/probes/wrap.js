/* B55 — long unbroken text must wrap, not run over the columns beside it.
   Reproduces the reported case: a programme item whose name is one long run of
   characters with no space in it covered the START time. Measured, not eyeballed:
   for every free-text cell, does its ink stay inside its own grid track, and does
   it stay clear of the START and END cells on the same row. */
...
const boot=async(b,cfg)=>{
  ...
  await p.evaluate(()=>go(CUR||'viewsched')).catch(()=>{});
  return {p,ctx};};
...
for(const cfg of [{n:'W390 phone',w:390,h:844,touch:true},
 ...
 for(const page of ['viewsched','editsched']){
  ...
  const r=await p.evaluate(([JAM,JAM2,pg])=>{
    const d=DAYS[0];
    /* the reported row, verbatim: a name with no break opportunity in it, and a
       second one carrying the jam in its sub-line */
    d.allhands=[{prog:'SODB '+JAM,sub:'',str:'0745',end:'0815',who:[]},
                {prog:'FLIGHT SAFETY STAND-DOWN '+JAM2,sub:JAM,str:'0830',end:'0900',who:[]}];
    d.notes=['EP: '+JAM];
    /* and the same jam through every other free-text cell in the day */
    ...
    INPUTS.push({person:'nobody-'+JAM.slice(0,12),date:d.dt,allday:true,s:0,e:1439,
      type:'Meeting',remarks:JAM,mod:''});
    validate();
    weekDirty(null); renderSchedule(pg==='editsched'?'eWeek':'vWeek',pg==='editsched',true);
    ...
    /* 2 · and must not reach into the START / END cells on its own row.
       Measured on the INK, not the cell: in a grid the cell keeps its track width
       while the text paints straight over the neighbour, so a rect comparison on
       the cell itself would see nothing wrong. A Range over the contents gives the
       real extent of what is drawn. */
    const inkRight=el=>{const rg=document.createRange(); rg.selectNodeContents(el);
    ...
    day.querySelectorAll('.ah-row,.pl-row').forEach(row=>{
      ...
      if(ink>first.left+1)out.collide.push((nm.textContent||'').trim().slice(0,16)
        +` (+${Math.round(ink-first.left)}px)`);});
    /* 3 · the row has to have grown taller instead — i.e. the text is really there */
    const jamRow=[...day.querySelectorAll('.ah-row')]
      .find(r=>/shahdbsbsna/.test(r.textContent||''));
    ...
    return out;},[JAM,JAM2,page]);

  const tag=`${cfg.n} ${page==='editsched'?'edit':'view'}`;
  T(`${tag} · no free-text cell overflows its box`,
    r.overflow.length?r.overflow.slice(0,3).join(' | '):'none','none');
  T(`${tag} · no name reaches into the START cell`,
    r.collide.length?r.collide.slice(0,2).join(' | '):'none','none');
  T(`${tag} · the jammed row wrapped onto several lines`,
    r.lines>26?'wrapped':`one line (${r.lines}px)`,'wrapped');
...
for(const cfg of [{n:'W390 board',w:390,h:844,touch:true},{n:'W1500 board',w:1500,h:950}]){
 ...
 const r=await p.evaluate(JAM=>{
   ...
   validate();
   ['sbDays','sbBoard','sbWarn','sbInputs','sbRoster'].forEach(id=>{
     const e=document.getElementById(id); if(e)e.__html=null;});
   renderScheduler();
   const out={overflow:[],panels:[]};
   /* free text on the board: anything that is not an input the user can scroll */
   ...
   /* and no panel may be pushed into a sideways scroll by the text inside it */
   ['sbBoard','sbWarn','sbInputs'].forEach(id=>{const e=document.getElementById(id);
     if(e&&e.scrollWidth-e.clientWidth>1)out.panels.push(id+' +'+(e.scrollWidth-e.clientWidth));});
   return out;},JAM);
 T(`${cfg.n} · no board text overflows its box`,
   r.overflow.length?r.overflow.slice(0,3).join(' | '):'none','none');
 T(`${cfg.n} · no board panel gains a sideways scroll`,
   r.panels.length?r.panels.join(' | '):'none','none');
 await p.close();await ctx.close();
}

console.log(`\n${pass} passed · ${fail} failed`);
await b.close();console.log('wrap done');})().catch(e=>{console.log('ERR',e.message);process.exit(1);});
