...
- Reduced motion: `scheduler.css` already blankets `*{animation:none!important}`. The land class HOLDS for `LIFT_LAND_MS` and a JS timer removes it (feedback stays, motion does not); `animationend` never fires there, so the timer always clears, and under normal motion both clear.
