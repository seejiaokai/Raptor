...
| NP5 | Approving offers New person, filled from the request, the admin's corrections winning; a typed callsign that is someone's opens "On the roster", named by his callsign, and is never picked for the admin — and when that person already has an account (archived or not — the account is checked first), the note says so and names both ways out (him on a new sign-in: change that account's sign-in; someone else: New person) rather than asking the admin to pick him (Fable's code read #1, both fix checks); an account's editor keeps its own person in the picker even once he is archived (Astra's fix check #2); Cancel discards | D214, D204 | `state/accounts.test.ts`, `ui/accounts-newperson.test.tsx` |
