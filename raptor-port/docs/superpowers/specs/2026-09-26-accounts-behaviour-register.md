...
| NP5 | Approving offers New person, filled from the request, the admin's corrections winning; a typed callsign that is someone's opens "On the roster", named by his callsign, and is never picked for the admin — and when that person already has an account, the note says so rather than asking the admin to pick him (Fable's code read #1); Cancel discards | D214, D204 | `state/accounts.test.ts`, `ui/accounts-newperson.test.tsx` |
