...
| NP4 | The sign-up asks what the admin's form asks — displayed callsign/name, initials (never required), pilot / WSO / personnel, CAT — and refuses a missing pick or an over-long name with its reason, never cutting it; personnel hold no CAT, so a CAT picked for a pilot never survives a change to Personnel (26 Aug 26; Astra's code read #2); it never tells a person not yet let in whether a callsign is taken | D214, D222, D225, D226, D204 | `state/accounts.test.ts`, `ui/accounts-newperson.test.tsx` |
| NP5 | Approving offers New person, filled from the request, the admin's corrections winning; a typed callsign that is someone's opens "On the roster", named by his callsign, and is never picked for the admin — and when that person already has an account (archived or not — the account is checked first), the note says so and names both ways out (him on a new sign-in: change that account's sign-in — another admin's job when it is the signed-in admin's own, and restore him on Quals if he is archived and back; someone else: New person) rather than asking the admin to pick him (Fable's code read #1, both fix checks); an account's editor keeps its own person in the picker even once he is archived (Astra's fix check #2); Cancel discards | D214, D204 | `state/accounts.test.ts`, `ui/accounts-newperson.test.tsx` |
