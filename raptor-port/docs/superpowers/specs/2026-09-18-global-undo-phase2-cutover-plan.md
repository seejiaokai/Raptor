...
# Rev 3 — reconciled round-2 dispositions (18 Sep 26, Fable + Codex both REVISE→narrow) — THE BUILD SPEC

Round 2 (both reviewers, resumed with the Rev 2 dispositions) confirmed the engine fix E1, the reland placement, the barrier retarget, the epilogue and the message mapping, and converged on a small set of corrections to Rev 2 itself. Both say: make these narrow edits, then build (no third full round; the standing post-build dual code inspection is enough). Author resolved the one cross-reviewer disagreement (redo-of-publish sign state) against the code. **This section is the definitive build spec; where it differs from Rev 1/Rev 2, Rev 3 wins.**

## The corrections (both reviewers convergent unless noted)

**C1 — Ineligible entries stay CONFLICT BARRIERS; only SELECTION skips them (Fable N2b, Codex R2-001).** Rev 2's "skip ineligible entries in the non-linear check" is unsafe: a newer ineligible entry sharing an authored key K with an eligible candidate would let the older undo apply a stale before-image over K (the SEQ-003 hazard). **Rule:** ineligibility removes an entry from `newestUndoable`/`mostRecentlyUndone` (selection) ONLY; `undoConflict`/`redoConflict` still see it and **refuse-whole** when its key set intersects the candidate. Test: eligible E1(seq5) touches K; newer INELIGIBLE E2(seq7) touches K + `people`; undo AND redo of E1 must REFUSE (not silently apply).

**C2 — Defer `lw.postouts` ONLY, not `lw.config` (Fable N2a).** `lw.config` restores cleanly (`applyLwRecord` restores it fully, `store.ts:1159-1170`; `reprojectRoster` re-reads `personEdits` on the next notify, `sync.ts:1003`), and it carries figure-hide, rearrange order, group colours, show-SANS, counters — deferring it would regress LW settings undo AND break `e2e/leavewar.spec.ts:1697-1740` (Undo ×5 figure hides). **Deferred-collection set = `{lw.postouts, people, settings, trk.*}`.** (A leave approval writes `lw.bid` states only, not `lw.config` — Rev 1's claim was wrong.)

**C3 — Restore reland uses `reconcileDayFiling(di)` per affected day, TWO-WAY, never auto-land (Codex R2-003, refines Fable N3).** `reconcileLandedAcc` is one-way (sets only MISSING acc); a days-only inverse that REMOVES a ground row leaves the live input's `acc='g'` dangling. Use `reconcileDayFiling(di)` (`slots.ts:477-496`) which BOTH clears a dangling `'g'` and restores a missing `'g'`. Steps in the restore, inside `schedWriteRecords` before `applyEnd()`: (1) strip `acc==='g'` from a CLONED input value (never the recorded inverse — GU2-009 aliasing); (2) for every day record in the write set, `reconcileDayFiling(di)`; (3) never call `autoAcceptInput`/push a row. **Expand coverage:** enumerate the inputs implicated by the restored day records and add their keys to the restore's authorization + `expectedRevs`/conflict set BEFORE the reducer commits (they are not in the original `changes`). Apply the same on REDO (symmetry). The undo-of-publish "input filed while published stays unlanded until next week-load" case matches the Unpublish button (`commitUnpublish` runs no reland) — the two converge.

**C4 — Button refresh via a TIMELINE-OWN store, NOT domain notify (Codex R2-002).** Rev 2's "notify both stores" would run the scheduler + LW reconcilers (`sync.ts:1154-1186`) on every record/undo/redo, enqueuing projections during phase-9 delivery and altering the entry's closure/eligibility. **Instead:** the timeline exposes `subscribeUndo(fn)` + `getUndoVersion()` (its own tiny external store, bumped on record/undo/redo); `Shell`, `SchedBoard`, `Chrome` subscribe to THAT (via `useSyncExternalStore`) in addition to their domain store, for the button disabled/label state. No domain notify from the timeline.

**C5 — Undo-of-publish: verified sign behaviour + strict enlist/mutate order (Fable N5, Codex R2-004/R2-005; author-verified).** `setDayApproved` does `signClear(di)` at publish (`publish.ts:202`) BEFORE capturing the issued snapshot, so `entry.forward` carries `dayOK=1` + CLEARED signs. Therefore **redo of a publish lands published + sign-cleared** (the forward image already has cleared signs — no active re-clear). **Undo of a publish** restores the before-image (which is SIGNED, since a day must be signed to publish, `publish.ts:194`), so it must OVERRIDE to `signClear(di)` per §6.2. A per-entry mutating restore hook (`adjustRestore(entry,dir,entries)`) runs INSIDE the reducer with this strict order (Codex R2-005 rollback safety): (a) **pre-enlist EVERY store in the closure** (publish closures can be multi-store — OIL/LW wake); (b) acquire both legacy locks (`reinstallLocks`); (c) append the `sched.retired` snapshot from the LIVE issued record, `logged = issuedDisclosed(id)`, **clearSigns:false at this step**; (d) the inverse write (book + delete the issued record; `applyBook` does not touch `sched.retired`); (e) `signClear(di)` LAST (so the book put at (d) cannot re-install signs). A `logged:true` retired record is append-only — an inverse never deletes it (Codex GU-P2-001).

**C6 — Per-issuance `id~n` disclosure is DEFERRED to Step 5 (Fable N6, Codex R2-007).** `n` is minted only at retire (`nextRetiredN`, `publish.ts:659-663`); a live issuance has only the reusable label, so `id~n` cannot be keyed at Step 3. Since NOTHING is disclosed at Step 3 (no DB), the label key has no live effect. **Decision:** keep the label key for phase 2 with the documented GU5-004 caveat; do NOT build issuance-identity minting now — it is Step-5 work. Written here so the builder does not invent it.

**C7 — Off-week undo: apply the stash while the week is OFF-SCREEN; pre-check before snap (Fable N4, Codex R2-006).** With snap-before-apply, `loadContext(weekstash/<wk>)` would `loadWeek(wk)` first, making `wk===CURWEEK`, and the CURWEEK guard then refuses AFTER the view moved. **Fix:** (a) `loadContext` IGNORES a context that comes ONLY from a `weekstash/<wk>` key (the stash IS the off-week store — don't load the week to write its stash); (b) an applicability pre-check runs BEFORE `snap` (every collection has a registered store; refusals happen before the view moves); (c) the off-week inverse is applied via `writeStashRecords` while the week is still off-screen (`id !== CURWEEK`), so the CURWEEK whole-refuse guard is only a safety net. **Fallback** if a clean off-screen apply proves too involved for phase 2: whole-refuse an off-week undo with a plain message ("switch to that week to undo this"), decided before the view moves. Prefer the off-screen apply.

**C8 — MINOR (both):** `foldProjection` must never let a folded projection's `scope` override the root entry's `scope`; keep the root's. `LW_COLLS` including `lw.current` is fine (nav is never restored). E1's stop-at-restore is safe with no legitimate-projection loss (both verified) — document the fixpoint bet: a restore-caused projection updates `expected` without a barrier, correct only while the reconciler is a fixpoint (the design's stated bet, §3.4).

## Build order (test-first; each its own commit; focused test per step; five gates once before PR; then dual post-build CODE inspection)
1. **2.0e-1** E1: `resolveRoot` stops at `origin==='restore'|'seed'`.
2. **2.0e-2** E2/C1/C2: deferred-collection eligibility exclusion + ineligible-stays-a-barrier (selection-only skip).
3. **2.0e-3** C4: timeline `subscribeUndo`/`getUndoVersion` version store.
4. **2.0e-4** E5: `inputs` Scope gains a week id; merge scope-derived context into `UndoEntry.contexts`.
5. **2.1** C3: reland-in-restore (`reconcileDayFiling`, clone-strip, expanded conflict/auth coverage) in `schedWriteRecords` before `applyEnd`.
6. **2.2** C5/C6: undo-of-publish `adjustRestore` hook (enlist→lock→retire→write→signClear); `logged` append-only; label-key disclosure + caveat.
7. **2.3** wiring in `main.tsx`: `installUndo` + registrations + `setCutoverModules(['sched','lw','inputs','plan'])` + hooks (`reinstallLocks`, `loadContext` ignoring weekstash-only, `snapView`, `showBubble`, `resolvePublishDay`); CURWEEK stash guard; restore epilogue (armDrop/prunePreviews before reflow; LW historyEpoch); one authoritative `LW_COLLS` (9); post-snap conflict re-check + plain-word refusals.
8. **2.4** retarget entry points (Shell/SchedBoard/Chrome→globalUndo/Redo, subscribe to the timeline version) + `w.undo/redo/histApply/histPush` + update `probes/adapted/audit-async.cjs`; unreachability test.
9. **2.5** Unpublish button UI (§6.4 gate incl `!protectedWeek()`) + OIL-credit warn (§6.6).
10. **2.6** off-week stash apply (C7).

Both reviewers: ready to build with C1–C8 folded in. Proceeding to build (Opus, high, test-first), then the standing dual cross-provider CODE inspection of the finished diff before the PR.
