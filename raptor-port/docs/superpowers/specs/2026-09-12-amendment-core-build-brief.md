<!-- CORE-BUILD SCOPE PREFACE (12 Sep 2026) -->
> **This is the frozen governing spec for the CORE amendment-engine build.**
> It is the Rev 5 brief. The CORE build implements the Rev 3-converged model
> (twice cross-provider-reviewed: Astra/Codex + Fable/Claude) PLUS the four
> must-fixes and the durability/undo/crew contracts.
>
> **DEFERRED to a separate follow-on item `[EOD]` (owner, 12 Sep 2026):**
> - **§3b EOD** (End-of-Day "record actuals" — the unsigned end-of-day publish).
>   The Rev 5 confirming review (Astra/Codex REV5-01…05) showed EOD needs its own
>   design cycle: an unsigned publish path in a clock-free app has no reliable
>   "this day is done" boundary (future-day abuse; worked-but-unclosed days;
>   frozen-eligibility). Split out to ship the converged core; design EOD later.
> - **The Rev 4 OIL simplification's EOD half.** For THIS build the OIL rule is
>   the simple core slice (see §6-CORE below), and the worked-day LOCK remains the
>   separate `[OIL]` backlog item exactly as planned.
>
> **§6-CORE — the OIL rule for this build:** each day's OIL derives from that day's
> **latest published record (latest AL, else Original)**, **per-day, never globally**
> (a change to day X never touches day Y — the original bug), keeping the read-failure
> protection (AM-02 `migration-failed`/`insufficient-evidence` must not wipe credits).
> The "lock earned OIL on an already-worked day" work stays `[OIL]` (post-AMEND).
> Everywhere §3b/§6 below say "EOD", read "the latest AL/Original" for this build.
>
> Build contract & phase order: `2026-09-12-amendment-core-build-plan.md`.
> Full design record: `2026-09-11-amendment-model-decisions.md`; EOD design lives in
> §3b below, preserved for the `[EOD]` follow-on. Review log (frozen shas, findings,
> dispositions) is kept with the build plan.
...
**Rev 5 (12 Sep 2026) — resolves the Rev 4 cross-provider review (Astra/Codex
REV4-01…REV4-05; plan names passed clean).** Folded in: EOD skips ONLY the four
signatures and keeps every other publish gate, is scoped to a day the scheduler has
deliberately CLOSED, and can never carry an unsigned future-plan change (§2, §3b —
REV4-01); EOD does factual-record validation, not prospective-plan approval, so a real
operational breach (e.g. crew rest) is RECORDED with an acknowledgement rather than
blocked (§3b — REV4-05); OIL reads ONE "latest committed version by per-day sequence,
regardless of kind" selector, shared by OIL / current-issued rendering / history, so a
correction is never ignored (§3b/§6 — REV4-03); the OIL sole-truth rule and the
`[OIL]` worked-day lock are UNIFIED — a worked/closed day changes only via an EOD
correction, so earned OIL is protected structurally, no separate cutoff (§6 —
REV4-04, owner-confirmed 12 Sep); and acknowledged (non-publication-gated) weekend-work
input claims are folded into the issued record explicitly, counted once (§6 — REV4-02).
...
- **Signatures: all four (CUR/SKED/PLAN/APPR) re-sign every amendment, bound to
  ...
  actuals-record publish skips ONLY the four signatures — it keeps every other publish
  gate (digest, base/revision verification, backup review, durable commit) and cannot
  carry an unsigned future-plan change (§3b).**
...
## 3b. EOD (End of Day) — the actuals record (owner, 12 Sep — SETTLED; Rev 5 incorporates the Astra/Codex REV4 review)
...
- **No four-role sign-off — an EOD just publishes** (owner): it is a record of fact, not
  ...
  (§2, AM-06).
- **INTEGRITY GUARD — the day-close boundary (REV4-01, resolved Rev 5).** Because EOD
  skips sign-off it MUST be a *record of actuals*, never a backdoor for an unsigned PLAN
  change. The scope is anchored on an explicit act, NOT a wall-clock "today" (the app has
  **no clock by design** — `ui/weeknav.ts TODAY` is a fixed literal; `engine/time.ts`
  rolls overnight ends into the next day, so a time-of-day test is unsound): **EOD is
  offered only for a day the scheduler has deliberately marked CLOSED** via the
  "Close day — record actuals" action, and a closed day accepts only EOD corrections
  thereafter — a future-facing plan change is NOT an EOD and still requires a signed AL.
  The exact close-marker mechanism + the "this edit is a deviation-record, not new plan
  content" rule are pinned in §12. A plan amendment (future/plan content) always requires
  all four signatures.
- **EOD validation = factual record, not plan approval (REV4-05, resolved Rev 5).** AM-06
  hard-blocks at every publish entry point, but an EOD must be able to record what
  *actually* happened — including an operational breach. So for an EOD, **structural /
  date / provenance / concurrency failures still BLOCK, but operational-rule violations
  (e.g. `CREW_REST`, emitted `'hard'` at `validate.ts:398`) are PERSISTED with a recorded
  acknowledgement instead of blocking** — the scheduler is never forced to falsify the
  record to publish it. The exact factual-vs-prospective split per warning code is pinned
  in §12 (extends the AM-06 hard/acknowledge matrix).
- After an EOD the day shows a **"recorded"** state but is **still correctable** (owner):
  a correction is a fresh EOD that supersedes it (never in-place, no retraction — F-01);
  OIL and history re-read the new latest by the §6 selector.
...
- **OIL & history read the day's latest committed version by per-day sequence,
  regardless of kind (REV4-03, resolved Rev 5)** — Original / AL / EOD, whichever is
  genuinely latest; NOT "EOD-if-present", which would make a later signed-AL correction
  be ignored. ONE selector, shared with §6 and current-issued rendering. Uses the **same
  immutable-publish machinery**.
...
- **OIL — the latest published record is the sole truth (owner, 12 Sep; Rev 5 folds in
  REV4-02/03/04):** each day's OIL is derived ONLY from that day's **latest committed
  version by per-day sequence — Original / AL / EOD, whichever is genuinely latest**
  (the ONE selector of §3b/§5, NOT "EOD-if-present" — REV4-03). If that record shows a
  person working a weekend/PH they earn OIL; if it removes them, they don't. **Negative
  balances are allowed.** OIL **derives per-day, not globally** — a change to day X never
  touches day Y's OIL (the original bug). For future days read the **issued/EOD resolved
  membership** (AM-04), not a live `availableFor` re-expansion (F-10).
- **Worked-day protection is STRUCTURAL, not a separate cutoff (REV4-04, owner-confirmed
  12 Sep — UNIFIES §6 with `[OIL]`).** A day the scheduler has CLOSED (§3b) changes only
  via an EOD actuals-correction, never via a future-facing plan AL — so the latest record
  of a worked day is always its actuals, and earned OIL on it can move ONLY through an
  explicit "didn't actually work it" EOD correction. That is exactly the `[OIL]`
  (11 Sep) lock + its stated exception, reached without a date test. The `[OIL]` backlog
  item therefore does NOT contradict this build; it is the Leave-War-side execution of
  the same rule (sequenced after, `/OUTSTANDING.md`).
- **Acknowledged weekend-work claims are folded in explicitly, counted once (REV4-02).**
  Today OIL pools TWO sources: the published schedule AND separately-acknowledged,
  non-publication-gated Duty-&-commitments input claims (`row.oil`, the OilConfirm
  ask-flow; `engine/oil.ts` skips ground rows carrying `src`). The sole-truth record must
  **account for both**: an accepted-input day's work evidence + acknowledgement is frozen
  into / reconciled with the issued record and counted exactly once; an EOD correction is
  authoritative over it; live-`INPUTS` dependence is dropped from published-date credit
  reconciliation **only after** the claim evidence is preserved. Wiring rides the
  existing `runOilPass`/`desiredOilCells` pass (`leavewar/sync.ts`), not a new read path.
- **KEEP the read-failure protection:** a saved week that can't be read (AM-02
  `migration-failed` / `insufficient-evidence`) must NOT be treated as "no work" and wipe
  credits — preserve until it can be read.
...
6. **The `navigator.locks` lease + backend revision-stamp** wiring and its quota/size
   gate thresholds.
7. **(Rev 5 / REV4-01) The EOD day-close boundary** — the close-marker mechanism and the
   "this edit is a deviation-record, not new future-plan content" rule; pinned by tests
   that a closed day accepts only an EOD correction, that EOD is not offered for an
   un-closed future-facing change, and that no EOD path issues content without the four
   signatures unless it is a factual correction to a closed day.
8. **(Rev 5 / REV4-05) The EOD factual-vs-prospective validation split** — which warning
   codes BLOCK an EOD (structural / date / provenance / concurrency) vs PERSIST with a
   recorded acknowledgement (operational, e.g. `CREW_REST`); extends item 3's hard/
   acknowledge matrix with the EOD column, pinned by a test that a true crew-rest breach
   can be recorded, not blocked.
9. **(Rev 5 / REV4-03) The single latest-committed-version selector** — "latest by
   per-day sequence, regardless of kind (Original / AL / EOD)", shared by OIL,
   current-issued rendering and history; pinned by fixtures for Original→EOD→signed-AL-
   correction and AL→EOD so no later correction is ignored.
10. **(Rev 5 / REV4-02) The acknowledged-claim reconciliation** — how `row.oil`
    ask-flow claims are frozen into / reconciled with the issued record (counted once,
    EOD authoritative, live-`INPUTS` dependence dropped only after evidence preserved);
    pinned against the `runOilPass`/`desiredOilCells` dual-source paths.
