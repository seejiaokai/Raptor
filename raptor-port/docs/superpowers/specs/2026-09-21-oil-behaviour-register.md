...
**WHAT OIL IS (owner, D25, 22 Sep 26).** Off In Lieu: a man who works a weekend or a public holiday earns TIME OFF — a full day (FO) or a half day (HO) banked as leave for him to take later. It lands on the Leave War as a cell and moves his OIL balance. **Nothing is paid.** The word "money" appears in this repo's comments and in the bug-check order as shorthand for "the consequential number a wrong answer changes"; read it as earned leave everywhere, and do not write it that way to the owner.
...
| OIL31 | What earns nothing by default: an SC spare · AVALON and BB, desks included (including a duty block MADE from the AVALON template — D20) · anything cancelled · a row with no readable times · a zero-length row · an ⓘ row · sim free text. **SUPERSEDED IN PART 22 SEP 26 BY D24 — not yet built, queued as `[OIL-EXEMPT-CREDITABLE]` in `OUTSTANDING.md`.** The three exempt KINDS — SC spare, AVALON (lines and desks) and BB lines — keep this default exactly, but stop being inert: the mode must OFFER the switch on them so an activated one can be credited, on a published day through an ordinary amendment. The word that matters in this row is already **by default**; what D24 changes is that today there is no way to say otherwise. All three are skipped before anyone enters the calculation (`oil.ts` — `saExemptKind`, `f.spare`/`ac.spare`), so no item key and no person-window exist for a credit to attach to; letting them in while defaulting off is engine work, not a flag. Everything else in this row — cancelled, unreadable times, zero-length, ⓘ, sim free text — is untouched by D24. | **REVISED 22 Sep 26 by D18** — a ground row that came from an input no longer earns nothing as a whole. The row is SPLIT: the man who FILED the request is governed by his own answer (the input half, OIL10), and anyone the SCHEDULER puts on that row beside him is ordinary scheduled work with his own per-person override. The requester never passes through the schedule half, so his own No cannot be overridden and he is never paid twice. |
...
| OIL41 | **The mode is READ-ONLY, and that reaches the crew panels and Undo (fix 5, 22 Sep 26).** Inside OIL Earn a claim's row keeps its switch and its puck live and shuts everything else — no typed time, no dropped puck, no Accept, and the late mark reads as a badge instead of a button. A puck cannot be dragged onto the day at all. **Sign and Publish stay open** (both reviewers). **Undo is NOT shut**: it walks back OIL decisions freely, because taking back a mis-tap is what it is for — but opening the mode marks the spot, and the press that would reach PAST it closes the mode instead of changing the day underneath a screen that says the day cannot change. Unpublishing and the plans selector close the mode too: both replace the day it is describing. |
| OIL42 | **A day that STOPPED being a holiday after it was published says so (fix 4, 22 Sep 26)** — the mirror of the advisory the forward case already had. It goes on paying off its frozen block, which is right, and the day now explains why and how to withdraw it. A weekday rule in practice: a Saturday cannot stop being a weekend. |
