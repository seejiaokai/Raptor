...
| Existing browser data | Converted by one function `migrateIds(c)` gated by `v3:<c>:idmig` — for EVERY course at `init` (`migrateAllCourses`), and again as a belt when a course opens. **Resumable and verified** (review, 10 Sep 26): `sSet` swallows a failed write, so the mapping is derived from every roster first (an entry lends its id, a string mints one), each record moves under its id and is read back before the legacy key goes, the roster — the index — is written LAST, the flag only when everything verified; a retry finishes an interrupted run. `collectStudents` converts an unfinished course on the fly so an export never drops a link. |
| One enrolment per course | `+ Add` looks the person up across EVERY roster of the course (`findEnrolment`: person id first, then name) and reuses the id and the name — pace and lulls are filed under it. A picked person whose callsign is already on the course under a DIFFERENT person id is refused with a message. |
| Import conflicts | `upgradeCourseBlock` re-keys only syllabi whose roster was strings, looks ids up by own-property, and throws on one id twice on a roster, one name twice on a roster, or one id under two names across syllabi; `fileFormat.js` refuses the same three before anything is written. |
...
| Where row ids are minted | ONE function `ensureRowIds(DAYS)` (`src/engine/rowids.ts`): walks the week, mints a missing `rid`. Called before EVERY baseline and snapshot — `initStore`/`loadWeek` before `weekBaseline` (a mint after the yardstick would make a pristine week read as edited and persist it), `histInit()` and `histPush()` before the snapshot — so every undo step, stash and persisted week carries ids, and nothing is rendered or saved before a row has one. No add path is touched individually. |
| A copy vs. the same row (review, 10 Sep 26) | **A copy is a new row:** a day template carries no ids (`mintBlob` strips them) and `applyDayTpl` strips as a belt; duplicating a day mints fresh ids on the PARKED draft, which is the copy. **The same row keeps its id:** a move splices the same object; the day on screen keeps its ids when it is duplicated, so the issued snapshot and every AL snap of an untouched day still name the live rows; undo/redo restore snapshots that carry ids; a draft switch and an amendment restore install snapshot rows with their ids. The first-seen dedupe in `ensureRowIds` stays only as a safety net. |
| The amendment book written before ids | `backfillSnapshotIds(SCHED, DAYS)` runs once per boot/week-load after `ensureRowIds(DAYS)`: a snapshot row (`SCHED.orig`, every AL's `snap`, the drafts) without an id takes the live row's id at the same address or a minted one, written INTO the snapshot so it persists — restoring, undoing or reloading then returns the SAME ids every time. |
