...
> **BROUGHT ONTO `main` ON 21 SEP 26, TEN DAYS AFTER IT WAS WRITTEN. Read this
> box before the brief.** It sat on its own branch (PR #395) while the work it
> describes went ahead, so parts of it are now history rather than instruction.
> Nothing in the brief was edited — only this box added.
>
> - **"NO code yet" is no longer true.** The amendment CORE is built and live on
>   `main`, including the plans selector redesign. Where the brief describes a
>   contract and the shipped code differs, **the code is what the app does** —
>   check it before treating a line here as current.
> - **Rev 4's own note still stands:** those refinements (the OIL simplification
>   in §6, EOD in §3b, free-label plan names in §4) were never re-reviewed by
>   either provider. Treat them as the owner's settled intent, not as reviewed
>   design.
> - **§6's OIL rule is about AUTOMATIC credit only** — what the published record
>   earns — and it still holds. It says nothing about a HAND-TYPED credit, which
>   the owner has since ruled on separately: an award says a man is OWED days, not
>   that he was at work (behaviour register N13, 20 Sep 26), and on 21 Sep he
>   ruled that **an award and a worked day ADD UP** — see `OUTSTANDING.md`
>   `[OIL-AWARD-ADD]`, ruled and not yet built.
> - The two companion files (`2026-09-11-*`) came across with it.

**Status:** design-first, NO code yet — SEE THE BOX ABOVE; the core has since been
built. Settled model + build-ready contracts.
