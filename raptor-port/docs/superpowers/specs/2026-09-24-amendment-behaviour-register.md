...
| AM54 | **Who made a change is the signed-in person's CALLSIGN** (D166 (5), built by `[ACCOUNTS]` 26 Sep 26 — REPLACES D104's "the shared account ("Admin" / the member account) until the database brings personal accounts"); a change the page's edit record does not hold reads "earlier"; a change with no single cell (a removal, a reorder, a filing, what the day earns) names nobody. | 25 Sep 26 · **D104 → D166 (5)** · `ui/pendlist.ts`, `state/store.ts HOOKS.whoami` | LIVE |
...
| AM60 | **A published day keeps what it went out with — every change that would move its face waits for the admin** (owner, D177, D178, D179, 25 Sep 26; built 26 Sep 26). A member's input filed, edited, deleted or moved after publishing, a downchit, a quals or posting change, a rule setting, a neighbour day: the working copy moves, the issued face does not; the day reads pending in ONE item per act, the four fall (D103), and the next AL — or Unpublish and publish again — takes it in. Back to what was issued reads 0 (AM20). A load cannot put a member's input back, so it stays pending. Stays live: the "Working draft" view, one's own hidden LATE mark, the Leave War's "no period" reminder, the next-day crew-rest mark; drawn live but COMPARED: a man's CAT on a puck, a printed rule value (`[LATE-PUB-FACE-LIVE]`). | 25–26 Sep 26 · **D177, D178, D179** · `engine/publish.ts` (`inputAxes`, `warnDelta`), `engine/inputs.ts inputsOn`, `engine/validate.ts faceWarn`; pinned by `ui/latepub.test.tsx` and `engine/official-flags.test.ts` (CRPF-001, §4 flipped) | BUILT on `claude/leave-late-published` |
