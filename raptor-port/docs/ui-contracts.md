...
## The My Programme page (`ui/MyProgPage.tsx` — owner, 7 Aug 26)

One person's week as a vertical run of day cards — the page most of the
squadron actually logs in for: when do I fly, when do I report, what is
flagged on me. Phone-first in a measurable way: the grid is
`repeat(auto-fill, minmax(min(340px,100%),1fr))`, which collapses to a single
full-width column on its own below ~730px, so the page needs no media query
and can never scroll sideways (gated in `e2e/geometry.spec.ts`). On a
desktop the cards flow into columns and the page doubles as the scheduler's
per-person lens.

**"Me" is the view-as person (`ME`), not the login.** `SESSION` is a role;
there are no per-person accounts (HANDOFF's server bullet), so the page
repeats the topbar's View-as picker in its own header — the topbar select is
hidden on phones, and a scheduler flips it to read anyone. When real
accounts arrive, the login→ME mapping slots in here and nothing else moves.

**Nothing on this page recomputes a rule.** The timeline is
`dayEvents(di, ME)` — validate()'s own per-day per-person index — sorted by
start, each row wearing its kind's section colour on the left edge
(`fly`/`shift`/`sim`/`duty`/`ground`/`prog`); a sortie grows the sub-line
aircrew actually plan around: report · brief · T/O · land, straight off the
event's fields. His inputs covering the day render beneath (dashed while
pending, solid once actioned; a whole-day away input replaces the empty
state), then his warnings via `personWarns` in the ordinary `.witem` row
box, and the cross-day crew-rest mark via `traceOf` on the day that caused
it. React owns the page (it is not a dense surface), so every string it
prints is escaped by JSX interpolation — pinned with a hostile remark in
`myprog.test.tsx`.

**A warning row leaves the page.** Rows carry `data-myw="di.ix"` — their own
attribute, not `data-adv`, because that branch scrolls a week which is
`display:none` while this page is up. The `routeClick` branch sets the page
to the View week and hands the ordinary `jumpToWarn` the address; the
deferred scroll runs after the week's effect has rebuilt the days, so the
pan lands on a real layout. The cross-day row is addressed at the NEXT
day's warning, exactly like its week twin.

Empty states are honest: a day with nothing tasked says so ("free day" only
when no input covers it either), and the card head carries the day's
Published/Draft state off `dayApproved`.
