...
| **The counter's window** ([ALL-AVAIL-WINDOW], 23 Sep 26) | The movable, non-blocking window a placeholder's count chip opens — who is available, and (OIL Earn on) who earns; the app's THIRD transient surface | `ui/AvailWindow.tsx` (the one opener `openAvailWinFrom`, the board's and the week's taps both call it); state in `state/view.ts` (re-exported by `pops.ts`); stacking and the phone panel in `scheduler.css .availwin` | Reads EVERYTHING in the world its chip was drawn in (`html.ts withChipWorld` — working copy, version preview, the issued face's official flags, a view-page draft day's official flags): drift-seams are (a) a new face that draws the chip must stamp `data-oilver`/`data-oilofw` for the world it is in, (b) the crowd, the chip and the window share ONE resolver (`oilSentOf`), (c) its switch rides the board's own write, refusals and history sentence (`toggleOilPerson`, `oilPersonSays`, `oilItemHistName`), (d) it closes on page/week/session change (`setPage`, `VIEW_RESET`) and when its version stops resolving (`prunePreviews`). Contract: `docs/ui-contracts.md` §[ALL-AVAIL-WINDOW] |
| **A published day's frozen face** ([LEAVE-LATE-PUBLISHED], D177–D179, 26 Sep 26) | An issued version keeps the day's inputs (`snap.inp`) and warnings (`snap.w`); the issued face draws only frozen things, and a later change reads pending | `engine/inputs.ts inputsOn`/`withFrozenInputs`; installed by `ui/html.ts withDaySnap` (issued versions) and `engine/validate.ts withIssuedWeek` (+ `weekctx.ts windowInputs`); the face bundle `validate.ts faceWarn`; the comparison `publish.ts inputAxes` / `warnDelta` | Drift-seams: (a) **a new reader of "the inputs on a date" on a schedule surface or in the validator must call `inputsOn(dt)`**, never `INPUTS.filter(…inputCoversDate…)` — a direct read shows the live inputs on the issued face (`latepub.test.tsx` greps the covered files); (b) a new field on an input that the schedule DRAWS must join `inputs.ts inpDetailKey`, or editing it moves the face with nothing pending; (c) a new path that clones a snapshot must carry `inp`, `w`, `pa`, `rv` (`retireIssued` does); (d) a new warning that is an admin to-do rather than a published fact goes in `validate.ts LIVE_ON_FACE`; (e) a new surface that reads the issued face's warnings reads `officialWarn()` (the face), never `officialRaw()` (the detector). Contract: `docs/engine-rules.md` §Publishing ("A PUBLISHED DAY KEEPS WHAT IT WENT OUT WITH") |
...
| **Qualifications** | The Quals grid; the qual ladder the validator reads | `QualsPage.tsx`; `people.ts` (`p.quals`, qual rules in `validate.ts`) | ticks PERSIST (the page calls `persistPeople()`; corrected 17 Sep 26); drive `QUAL`/`SC_QUAL`/`AAR_*` checks |
...
| **Roles / auth** | Admin vs member vs view-only; who may write; the Admin page (23 Aug 26 — the template openers, the persistence honesty card). **Since `[ACCOUNTS]` (26 Sep 26): accounts — every account IS one callsign (D166); the sign-in's five outcomes (in / request access / waiting / switched off / guest — D204); Admin → Users (accounts, requests, the guest switch); the ONE permissions module** | `state/perms.ts` (the matrix, the named questions, the command gate, the ownership check) + `perms.test.ts` (§11 drift) + `perms-scan.test.ts` (no gate outside it); `state/accounts.ts`; `auth.ts`; `ui/AccessScreen.tsx`, `ui/GuestApp.tsx`, `ui/UsersPanel.tsx`; `AdminPage.tsx` (`#admDeny`) | checked at the PAGE, the WRITE path AND the command gate, never the nav; a new rule goes into `perms.ts` and `data-model.md` §11 together |
| **Leave War tab** | The vendored leave-bidding calendar — its own store, storage (`leavewar:` keys) and CSS (scoped `#page-leavewar`) | `src/leavewar/` (page seam: `LeaveWarPage.tsx`); boots once in `main.tsx`; role written only by `resetSession` + the admin's `toggleRole` (both write the session's EFFECTIVE role; **advancing the cycle stage is admin-only since 27 Aug 26** — `store.ts:advanceStage` refuses a member and `Chrome.tsx` hides the control; members otherwise bid exactly as before) | **the sync wires are LIVE (17 Aug 26)**: its roster is a boot projection of `PEOPLE`, approved LW leave lands in `INPUTS` as lw-tagged rows (so the Unavailable block, availability, warnings and the palette all move when a bid is approved), and leave filed on the Inputs page lands as Raptor-owned LW cells (so LW balances draw down). **Medical rides the same wires (17 Aug 26)**: an ATT B / ATT C / HL / OML input lands as a raptor-owned LW cell (AM/PM exact; a custom window ≤6h reads as a half day — `medRowPortion`, NOT leave's round-OUT rule) and an admin-marked LW medical cell lands as an lw-tagged input with no approval step. A change to Raptor's INPUT_META leave OR medical types, half-day boundaries, roster fields (seat/q/sxo), or the INPUTS write epilogue is a DRIFT-SEAM against `src/leavewar/sync.ts` — walk it. **The roster is a LIVE, CATEGORISED projection (18 Aug 26)**: `sync.ts:reprojectRoster` re-projects `PEOPLE` on every Raptor notify (so a new person appears without reload — added on Admin → Users since 26 Sep 26, Flow G; it was a Quals-page add), and `engine/people.ts:groupOf`/`catClass` group and colour the grid by SXO / CAT / OCU / ground crew — so a change to Raptor's **CAT ladder (`QORDER`/`QCOLOR`/`isInstr`), the `--q-*` colour tokens, the `pers`/`GND`/`flight` ground-crew shape, or `sxo`** is a drift-seam against `state/raptorRoster.ts` + `engine/people.ts` (the colours are duplicated from `scheduler.css --q-*` into `matrix.css`, kept in lockstep by eye). Ground crew ride the roster but MUST stay out of manning: `engine/availability.ts:countsFor` skips `pers` first — a change there that dropped the skip would silently corrupt every threshold. The same wire mirrors Raptor's "View as" person (`ME`) into Leave War's `viewer` (row highlight + the counter picker's "yours" numbers), so a change to `setMe`/ME semantics reaches Leave War too. **Publishing reaches Leave War too (wire 4, 17 Aug 26; REWORKED 28 Aug 26)**: publishing a weekend or LW-holiday day moves raptor-owned FO/HO cells (**corrected 17 Sep 26:** the old "publishing/reopening/re-issuing" is stale — reopen and re-issue were removed at Phase 2; a change to a published day is the next AL, which runs the same pass) (FS/HS before the 28 Aug 26 rename) and the OIL balance there (`runOilPass` derives from the issued snapshot; since the rework the rule is ONE ≤6h/>6h test on the day's start-to-finish ENVELOPE — gaps included, 29 Aug 26 — across SC MAIN, flying seats via report→debrief, sims, duties, ground and Common-Programme rows, the SC shift-window rule deleted; and since 29 Aug 26 the schedule half reads EVERY visited week via the week stash, not just the loaded one — the stash PERSISTS since 8 Sep 26, but note a week merely looked at and never changed was never stashed, so it contributes nothing), so a change to `setDayApproved`, the snapshot machinery, SC wave shapes, flying/sim/duty/ground/allhands written times, `VCONF.reportLead`/`VCONF.debrief`, or `VCONF.oilFullMin` is the same class of seam — `engine/oil.ts` + `docs/engine-rules.md` §Weekend/PH work earns OIL. **An acknowledged Duty-&-commitments input credits OIL — SINCE 21 Sep 26 ONLY ONCE THE DAY IS PUBLISHED (`[OIL-AUTO-REMOVE]`; the 28 Aug "without publish" is SUPERSEDED)**: input saved → `oilGate` (`ui/inputedit.tsx`, all three editors) → the OilConfirm sheet → `row.oil` day decisions (one `writeInputsBatch` with the save) → `desiredOilCells`' input pass pools them with the published schedule → FO/HO cell; an unanswered applicable day instead lights the bell (`oilPendingFor` → the Shell tap → InputEditor + the OIL sheet via `pops.OILASK`) — so a change to `oilAsks`' type set, `row.oil`'s shape, or the OilConfirm gate sites is a drift-seam against `leavewar/sync.ts` (`oilAskPlan`/`desiredOilCells`/`oilPendingFor` all read the one exported `isNonWorkingISO`). **THE OIL EVIDENCE BLOCK is the new seam and the biggest one (`[OIL-AUTO-REMOVE]`, 21 Sep 26, `engine/oilev.ts`)**: a day carries its own answer to what earns — the scheduler's decisions stored on `Day.oild` (day content) plus, FROZEN into the issued snapshot only, `Day.oilev` (a projection of every OIL-bearing input covering the date and the resolved membership of every sentinel event). `desiredOilCells` reads THAT and nothing live, so the drift-seam list grows by four: (a) anything that CLONES a snapshot day onto a working copy must strip `oilev` or it installs a stale projection (`drafts.ts:liveDay` — `loadVersionToWorkingCopy` and `draftSelect` are the two paths); (b) the block rides ONE aggregate delta address `oil:<di>` (`publish.ts:oilDelta`) and the signature binds it (`currentBind`), so a new publish/ signature/recovery path must carry that axis as it carries the input-filing axis; (c) `HOOKS.oilEarningDay` / `oilDayISO` / `oilSentinel` are the three facts the engine cannot know — installed by `sync.ts:installAbsenceDoor`, unset they mean "no day earns and a sentinel stands for nobody", which is a SAFE default and also a silent one; (d) the ITEM ADDRESS grammar (`engine/oil.ts` `inputItemKey`/`rowItemKey`/`groundItemKey`) is what a decision hangs on, so a path that re-mints a row's `rid`, or that lands an accepted input WITHOUT setting `src` to the input's id, silently drops the admin's decision. **THE UI SURFACES**: the board's OIL Earn mode (`ui/oilmode.ts`, `ui/board.ts` + `ui/board-html.ts`) and the green edge on the puck, drawn by `puck()`'s 7th argument through `ui/html.ts:oilSeatDeco` — ONE body for the week and the board, so a seat renderer that calls `puck()` without it shows a man no bar on a day he earns. **THIS SEAM OPENED ON THE DAY IT WAS WRITTEN (owner found it in the running app, 21 Sep 26).** The warning said "a NEW seat renderer"; the hole was in two EXISTING ones. The board does not use one seat builder — `sbSlot` (every cockpit seat, so every flying line and every SC shift) and the Common Programme's inline builder both draw their own markup, and neither asked for the decoration. The money was right throughout; only the mark was missing, so a whole Saturday of flying crews read as earning nothing. No unit test saw it because every bar assertion had been written against a duty desk or a ground row — the two that DO go through the shared builder. So the seam is not "remember to pass it in new code": it is **enumerate every call site of `puck()` and decide, for each, whether that seat can earn** — pinned now by `oilmode.test.tsx` §"the green strip reaches every kind of seat the board draws", which walks the seat KINDS rather than one panel. The bar must also survive a CHIP: the late/advisory chip is the puck's first child with a solid background over the same 4px, so `scheduler.css` re-draws the stripe on `.lchip` too. Weekend and holiday days only, so five days a week every one of these emits nothing. Contract: `docs/ui-contracts.md` §OIL on the schedule; rulings: `docs/superpowers/specs/2026-09-21-oil-behaviour-register.md`. **ONE absence record ([ARCH-STACK] step 4, 20 Sep 26 — supersedes the 17–18 Aug two-way copy)**: an approved or filed absence IS the Raptor Input; the war derives its cells on read (`sync.ts:refreshAbsences` → `state/merge.ts` → `engine/dayview.ts`), so an Inputs-page edit or delete needs no carry-back. A member's own date/type/person edit of a war-approved leave clears `lw` (it keeps its green, gains the blue Inputs edge); a remarks-only edit keeps it. Every Input write passes the owner's clash rules at ONE seat (`leavewar/inputgate.ts`): a change to any Input write path, to what a day view derives, or to the war's record list must keep that seat on the path (drift seam: a raw `INPUTS.push` in a production path skips the rules). **The synced-leave remark is the shared date tail (18 Aug 26)**: `sync.ts` mints `engine/inputs.ts:withRemarksTail(prior, start, end, 'on')` and the Inputs-page `withTill` (`ui/InputsPage.tsx`) calls the same helper with `'none'` — a change to the tail's wording/format is now ONE seam, not two, but the `single` argument's split (calendar suppresses a one-day tail to avoid a mid-pick flash; a settled synced leave shows "on 15 Jul") must be preserved. `withRemarksTail` rewrites the `till/on <date>` token WHEREVER it sits so a member's note before OR after it survives the date moving. **New input type `Duty` (18 Aug 26)** is grp:'act' — it behaves like Appointment in every derived predicate BY CONSTRUCTION (`isPersonal`/`canSpare`/`isUnavail`/…), so it does NOT sync to Leave War (only leave/medical do); nothing special-cases it, which is the point — a future grp change to it would silently move it. **Roster within-group order** is `engine/people.ts:CAT_RANK` (most-qualified first: FI, IR, IP, IW, A→D, OCU) — a display-only sort, `manning`/`categoryOf` untouched. **FL P / WM P manning rows (18 Aug 26)** split the pilots by CAT (`engine/people.ts:pilotLead` → availability.ts `flp`/`wmp` → seed rules, display-only at amber 0/red 0) — the ONE manning path that reads the CAT (`q`), so a change to the Raptor CAT projection or the FL/WM cat sets moves these two counts (a deliberate exception to "manning never reads q"). **The manning count rows are admin-arrangeable/hideable (18 Aug 26)**: `store.ts` `manningOrder`/`manningHidden` (persisted `manningorder`/`manninghidden`, admin-gated `moveManningRow`/`toggleManningRow`/`resetManning`), rendered by `ui/CountRows.tsx` under the same Rearrange toggle as the roster — display-only, the requirement rules and thresholds untouched. **The thresholds ARE squadron-editable since 19 Aug 26** (tap a row's name → `ui/ManningSheet.tsx`, admin-gated `setManningThreshold`/`resetManningThreshold`), so "display-only at amber 0/red 0" is a default, not a law. **CORRECTED 17 Sep 26:** the numbers no longer persist under `manningthresh`, and the rule definitions are no longer code-owned — since the same day's rework the whole rule set IS data, persisted WHOLE under `manningdefs` (`requirements.default.rules`, edited through `saveManningRule`/`deleteManningRule` plus the threshold setters). The old `manningthresh` overlay is still READ at boot so a squadron's tuned amber/red lines survive the upgrade, but it is NEVER WRITTEN again. **SC D / SC N team rows (19 Aug 26)**: `availability.ts:scTeams` counts complete six-body teams (2 SC pilots + 2 SC WSOs + 1 SXO + 1 crew) off `Person.scd`/`scn`, which the projection reads from Raptor's **`quals.scDay`/`quals.scNight`** — a SECOND deliberate qual read beside `q`, so a change to those Quals flags' names or semantics is a drift-seam against `state/raptorRoster.ts` + `reprojectRoster`'s signature; duty-standers count as PRESENT for these two rows only. **Post-out (PO, 18 Aug 26; reworked 19 Aug 26)**: `store.ts:setPostOut(id, fromDate|null, archive)` sets a person's posting-out date (`to = fromDate − 1`, ANY real date) plus the explicit `Person.poArchive` flag; the grey `.gone` hatch and the manning exclusion key off `to` as before. An admin taps a day (BidPicker's folded PO controls) to place it and a struck day (`PostOutSheet`) to move/flip/undo it; the last day in wears the `.polast` tag. **This is now a CROSS-APP FLOW**: `sync.ts:runPoArchive` archives the Raptor body when the date arrives (`poArchive === true` only — a seeded/demo `to` carries no flag and is never read as consent), the Quals page's Archived drawer restores through `sync.ts:restoreArchivedPerson` (clears the LW posting FIRST or the next pass re-archives), and `reprojectRoster` KEEPS an archived body that has a posting window. A change to `archived`'s meaning, to the projection's exclusions, or to the restore order is a drift-seam against all three. **The matrix roster is month-windowed (19 Aug 26)**: `Matrix.tsx` hides rows whose `[from,to]` miss every visible month, re-rendering only on row-SET changes and anchor-compensating `scrollLeft` after each (auto table layout re-narrows columns a removed row widened — the month-jump maths lands short without the correction; the phone mirror re-measures on the same signal). A change to row rendering, column sizing, or `jumpTo` must keep that anchor correction honest. **Event rows are variable-count (18 Aug 26)**: `DayInfo.events` is `string[]` and `EventBand.line` a plain index; `store.ts` `eventRows` (persisted `eventrows`, admin `addEventRow`/`removeEventRow`, cap `MAX_EVENT_ROWS`) drives how many `ui/EventRows.tsx` draws — a change to the event tuple/line shape, `columnKindFor` (now scans every line), or the read/write of `events` must keep the variable length; `dayEvent(day, line)` is the one bounds-checked reader. **Event tags are per-instance (18 Aug 26)**: `DayInfo.eventKinds[line]` / `EventBand.kind` override the type-library word match everywhere (`columnKindFor`, the red work word, wire 4's holiday answer — an instance 'off' tag makes a worked day earn OIL exactly as a library word does), so a change to classification must keep the instance-first precedence; the library changes only via Edit types. **The page is KEPT MOUNTED behind a memo firewall since 1 Sep 26** (`LeaveWarPage.tsx LwBody` + the Shell's `.doze` section — ui-contracts §The Leave War tab is kept alive): a Raptor notify no longer re-renders the LW tree at all, so ANY fact the LW UI renders MUST cross the seam through the LW store's own notify — rendering Raptor module state directly inside `src/leavewar/ui/` would sit stale behind the memo, a new drift-seam class to walk when adding a wire **The OIL TRACKER (2 Sep 26)** is a new surface inside the tab (`ui/OilTracker.tsx`, toolbar button `oil-tracker` for both roles + the Cinch sheet's OIL BAL row): it is DERIVED (`engine/oiltracker.ts` — FIFO oldest-credit-first, expiry per the admin's `oilPolicy`, persisted `oilpolicy`) and its admin writers are the first ledger writers (`grantOil`/`updateLedgerEntry`/`removeLedgerEntry`) plus `setOilPolicy` and `setBalance` (LVE BAL). Drift seams: `OIL BAL` everywhere now reads the tracker's balance through `store.ts:figureCtxOf()` — the column, picker, breakdown, Cinch row, tracker and the bid-time `wouldLeave` warning; a new figure surface must take its ctx from there. Earned FO/HO days stay grid cells (never ledger rows). `OFF USED` figure removed the same day. **Second cut the same day**: the tracker is a full-screen GRID (`Sheet full`, year lanes, one credit box per credit with its FIFO takes inside, `select.ts:wireRowSelect` on the shared `wireGesture` core — a change to the grid's drag feel constants now moves the tracker too); an earned credit's REASON is the FO/HO cell's `BidRecord.note` written by `ingestDutyCredit(…, why)` from `engine/oil.ts:dayOilWork`'s per-span kind (`FLT`/`SIM`/`Duty`, an input's type name) — so a change to which schedule rows count, or to their kind, is a seam against `oiltracker.ts`' reason line; `LedgerEntry.givenBy` (optional) rides `grantOil`/`updateLedgerEntry`; an admin's manual OIL/FO/HO write on the grid opens the tracker (`Matrix.tsx onWrote`). **Third cut**: a dead credit folds into the ARCHIVE column — a DISPLAY switch in the tracker, never a store fact, so nothing else reads it; `store.ts:installDemoOil` is a second boot-only writer beside `remapPersonKeys` (same contract — never for tests, never persisted, called before the re-key). **OFF is no longer a leave code (2 Sep 26)**: gone from `codes.ts`, `INPUT_META` and LVE USED; the management **Off day** is the `free` EVENT KIND (`eventdefs.ts`, grey `evfree` column, earns NO OIL — `isNonWorkingISO` reads only `off`), so a change to event kinds must keep `free` out of the OIL non-working test. **A weekend/PH day of leave CHARGES NOTHING (3 Sep 26)**: `leavewar/engine/charge.ts:chargedDays` is the ONE body deciding which counter-bearing cells draw (weekend, the war's `ph`, or an `off`-tagged event — through `eventdefs.ts:isNonWorkingDay`, which `sync.ts:isNonWorkingISO` now delegates to, so OIL-earning and leave-excusing read one holiday definition), with the pilots' 15-day run exception reading `Person.seat` — `drawnFrom`/`takenOf`/every USED figure, the OIL tracker's debits and `setBalance` all read it via `figureCtxOf()`'s `eventDefs` + `people`. Drift seams: a new figure or balance surface that builds its own ctx without those two fields silently loses the PH and pilot halves of the rule; a change to the seat projection (`raptorRoster.ts`) or to `columnKindFor`'s precedence moves what a leave day costs; manning is deliberately untouched (`removesAvailability` never learned the calendar). **CL (compassionate leave, 3 Sep 26)** is an LL twin in Raptor's `INPUT_META` and a `cl`-counter leave type in `codes.ts` (figures `CL BAL`/`CL USED`, admin-settable via the Cinch sheet's Set — every plain-sum balance figure names its `counter` now), crossing the sync wire by name like every other leave code. **The sheet SCRIM steps aside on a touch screen (6 Sep 26)**: `ui/Sheet.tsx useGridPan` sets `pointer-events: none` on `.sheetscrim` when `(pointer: coarse)` matches, so a finger scrolls `.mx-wrap` itself (the browser's own fling), and a document-level CAPTURE shield stops `pointerdown/up/cancel`, `touchstart/end/cancel`, `mousedown/up`, `click`, `dblclick`, `contextmenu` aimed under the sheet before React's root listener and the grid's own handlers, closing on the click — so a NEW grid interaction that starts from an event outside that list (a `keydown` on a focused cell, a `focusin`, a `wheel`-driven action), a grid handler registered on `window` or on `document` capture BEFORE the sheet mounts, or a sheet panel rendered without the `.bidsheet` class (the shield exempts targets inside one) is a drift-seam against the shield: the first would act behind an open sheet, the second would see the tap the shield meant to swallow, the third would have its own taps eaten. And anything that drives `.mx-wrap.scrollLeft` from JS PER FRAME while a finger flings it (a proxy scroller, a rAF follow) reintroduces the main-thread stutter the same day's first cut had — the 30 Aug no-mid-fling-write rule now covers followers too. **The DEFAULT VIEW is chosen by stage (7 Sep 26)**: at boot `store.ts` picks `currentId` via `engine/stages.ts:pickDefaultPeriodId` (open→closed→published→draft) unless a `current` is remembered, and `LeaveWarPage.tsx` lands the grid once on `engine/period.ts:defaultFocusDate` (`bidFrom ?? start`) the first time the tab shows — `selectWar` lands the same way on a picker switch. Drift-seam: this reads `period.stage` and `period.bidFrom`, so a change to what those MEAN (a new stage, clearing `bidFrom` on close/publish, a stage other than `open` wanting the box/priority) moves where the tab opens; keep it in step with the open-bidding box, which reads the same two fields. |
| **Tracker tab** | The vendored OCU Progress Tracker (7 Sep 26) — its own store (`tracker/app/core.js` subscribe/notify), its own storage doorway (`tracker/storage.js`, `ocu:` keys) and CSS (scoped `#page-tracker`, `tracker.css`) | `src/tracker/` (page seam: `TrackerPage.tsx`, kept mounted once visited); nothing boots in `main.tsx` — `core.init()` runs on first mount; role written only by `resetSession` + the admin's `toggleRole` through `tracker/role.js` (a no-import module, so Raptor never loads the chart engine to set it) | **One-way data crosses the seam since 9 Sep 26 — the PEOPLE BRIDGE** (`tracker/people.js`, a second no-import module; `tracker/peoplewire.ts` projects `PEOPLE` into it on every store notify, signature-guarded, and hands it `HOOKS.whoami()`; wired once by `TrackerPage.tsx`): the Students card's `+ Add` lists the squadron roster and a pick lands an enrolment entry `{ id, name, pid }` (students are keyed by that opaque id since 10 Sep 26 — every `:m:`/`:d:`/`pace:`/`lulls:`/`last:` key takes it; the 9 Sep 26 `v3:links` record is folded into `pid` by the once-per-course migration `core.js migrateIds` through `app/ids.js`, the one converter Import shares), and every mark/date write is stamped `by`/`at`. Nothing flows BACK to Raptor. Drift-seam (3): the projection filter (no archived / sentinel / ground) is `peoplewire.ts projectForTracker` — one body; the Leave War's `projectPeople` is a DIFFERENT projection with different rules (it keeps ground crew), do not merge them without the owner. Two older drift-seams exist: (1) the FILE LOCK — the file entry points in `core.js` (`importClick`, `openCopy`/`saveCopyClick`) open with `if (fileLocked) return` and `Header.jsx` hides the File menu for the same flag (owner, 7 Sep 26: everyone edits, only the file portion is the admin's), so a NEW way to reach a file must carry both halves, and `tracker.test.tsx` pins the count of guards and the header's list (since 9 Sep 26 the file is a FORMAT, not a store — Save changes writes flow edits to the store only, and a new surface must not re-bind a file); (2) the STYLESHEET — five Raptor class names (`.day`, `.day.today`, `.legend`, `.modal`, `.sub`) are reset at the top of the `#page-tracker` wrapper, so a new Raptor rule on a bare class the Tracker also uses (audit: intersect `tracker.css`/its JSX class names with `scheduler.css`) is a seam — the vendored smoke suite (`scripts/tracker/smoke.mjs`, 345 checks at 1440/390) is what caught the first five. The tab is a viewport-tall COLUMN: `body.tr-on` locks the document and drops Raptor's 120px body pad while it is up, and `--tr-top` is the section's measured top edge — so a change to the top bar's height, to that body pad, or to `.page`'s show/hide reaches `TrackerPage.tsx`'s measure. Two element ids were renamed on the way in (`#trUndoBtn`/`#trRedoBtn`); any new Tracker id must not collide with Raptor's (both apps share one document). |
...
gesture (drag / type)
  ...
  → noteChange(key)              tints the cell pending (NOT what decides the AL)
  → afterSchedMutate()
  ...
  → if the day is PUBLISHED: dayDelta sees the change → next AL → PUBLISHING/AL
```
**CORRECTED 17 Sep 26 — what puts a day in the next AL.** Not the pending marks. Since the
amendment core, `engine/publish.ts:dayHasChanges` reads straight through the canonical
`dayDelta`, "the one authority for eligibility, the panel counts and the stored diff". Two
consequences worth holding on to: a write that BYPASSES the funnel still reaches the next
AL — unmarked and unexplained, which is worse than being absent — and a mark on something
the delta cannot see (a reorder whose effective display order is unchanged) puts nothing in
the AL at all.
...
edit a signed day       → the canonical dayDelta vs the issued snapshot becomes the AL
                           (alIssue stores it as the record's `diff`). CORRECTED 17 Sep 26:
                           NOT "the pending keys become an AL" — pending marks only tint.
...
load a version onto the working copy
                         → loadVersionToWorkingCopy (engine/drafts.ts). CORRECTED 17 Sep 26:
                           restoreDayVersion was REMOVED at Phase 2 — it is not in src at all,
                           not even for the probe bridge, which exposes
                           loadVersionToWorkingCopy instead. Loading does NOT move SCHED.cur.
```
...
per-week stash added 23 Aug 26 — "session stash" until the 8 Sep 26 storage work made it
PERSIST; renamed here 17 Sep 26)
```
...
button there (`ui/adminopen.ts`). After the command seals, `notify()` carries the new `PEOPLE` entry to every projection
...
- **Persistence** — does the feature imply data surviving a reload? **CORRECTED
  17 Sep 26 — this item was written before the 8 Sep 26 storage work and had it
  backwards.** On a BUILT site almost everything persists: the config family (`rules`,
  `stores`, `dutytpl`, `daytpl`, `cxreasons` and the rest of the 11 settings keys), plus
  INPUTS, PEOPLE, the plan layer, every stashed week AND the live week (so publish state,
  the Originals, the AL list, parked drafts and `WARNOFF` all come back), the whole Leave
  War world, the Tracker's data, and medical documents in their own IndexedDB drawer.
  Genuinely session-only, by design: **undo/redo history, the edit log, and the view-state
  registries** (`LATEOFF`, the notification bell `BELLLIT`, the armed slot, selection).
  Dev and tests start clean on purpose (MemoryBackend). The authority is CLAUDE.md
  §Architecture rules "WHAT ACTUALLY PERSISTS"; check it, don't guess.
...
- **The Leave War batch writers vs their single-cell parents** (27 Aug 26,
  ...
  **The ROW guard `canEditRow` is the fourth face** (27 Aug 26 — "viewing as
  ...
  always mirrors the signed-in person, and `''` — no row — for someone signed in without access; `perms.ts viewerId`)
...
- **The two role reads (`LOGINROLE` vs `SESSION.role`) are GONE with the admin's view toggle** (`[ACCOUNTS]`, 26 Sep 26,
  D166 (3)): there is ONE role, the account's, read through `state/perms.ts`. The seam that remains: the Leave War's
  role is a second COPY (its store's `state.role`), written by exactly ONE production writer — `resetSession` — through
  `lwSetRole`; its `viewer` is the sync's mirror of `perms.ts viewerId()`. `leavewar/permsparity.test.ts` holds the war's
  writers to the same §11 table.
- **The accounts seam** (`[ACCOUNTS]`, 26 Sep 26). WHO is signed in reaches: every own-row rule (inputs, the Quals row —
  D149, the war row, undo's `mayReverse`), every "who" record (the edit log, bug reports, the Tracker's `by`, the
  replaced-bid notice, the ledger's approver, unpublish — the callsign shown, the person's id kept beside it), the purple
  "this is you" puck, the OIL bell. All ask `perms.ts` (`me`, `isMe`, `may…`); a new reader that compares a person with
...
- **Duties are decoupled from waves (13 Aug 26).** A duty desk is minted from a
  ...
  a removed seam. The template's `wave` field (7 Sep 26) is a LABEL the desk
  carries, not a link: it re-arms the engine's `sa` marker, and the wave and
  the desk still live and die separately. The template library is a new persisted-config surface beside
...
- **`weekStashSnap` (`state/store.ts`) shares its SCHED field list with
  ...
  accident.** Both serialize the same **fourteen** SCHED fields (corrected 17 Sep 26 —
  `schedFields` has 14, not 11) for two different
...
- **`engine/weekstash.ts`'s snapshot shape is `weekStashSnap`'s to change
  ...
  by `applyWeekModel` — the two live in one codebase, so they move together; the seam to
  respect is `schedFields` above. **CORRECTED 17 Sep 26:** the old "no persisted blobs means
  no stale-schema blobs" free pass is GONE — stash entries are persisted now, which is
  exactly why `storage/reset.ts` carries a `SCHEMA_VERSION` and clears `inputs`+`weeks`
  when a shape changes incompatibly. A shape change here needs that version bumped.
