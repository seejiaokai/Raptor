...
| **Leave War tab** | The vendored leave-bidding calendar — its own store, storage (`leavewar:` keys) and CSS (scoped `#page-leavewar`) | `src/leavewar/` (page seam: `LeaveWarPage.tsx`); boots once in `main.tsx`; role written only by `resetSession` | today it touches nothing else and nothing touches it — the day a sync wire lands (`docs/superpowers/specs/leavewar-sync.md`) this row grows the crossings; a change to Raptor's INPUT_META leave types or half-day rules is a drift-seam against that spec |
