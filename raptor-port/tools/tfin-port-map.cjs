...
const PORTED = new Map(Object.entries({
  ...
  "every day carries its own sign-off strip": 'ui/editweek.test.tsx',
  "the strip has the four roles": 'ui/editweek.test.tsx',
  "each role is a roster dropdown": '≈ ui/editweek.test.tsx',
  "the strip knows which day it belongs to": '≈ ui/editweek.test.tsx',
  "the view page shows no sign-off strip": 'ui/editweek.test.tsx',
  "publish day is locked while unsigned": 'ui/editweek.test.tsx',
  "the lock says what is missing": '≈ ui/editweek.test.tsx (title from signMissing)',
  "a locked day cannot be published by clicking": 'ui/editweek.test.tsx',
  "three of four is still locked": '≈ ui/editweek.test.tsx',
  "all four unlocks the day": 'ui/editweek.test.tsx',
  "day 1 keeps its own empty strip": '≈ ui/editweek.test.tsx',
  "per-day beak buttons": '≈ ui/editweek.test.tsx',
  "view beak read-only": '≈ ui/editweek.test.tsx',
  "part-published banner": 'ui/editweek.test.tsx',
  "day 0 marked ok": 'ui/editweek.test.tsx',
  "day 1 still draft": '≈ ui/editweek.test.tsx',
  "undo unpublishes the day": '≈ state/store.test.ts + ui/editweek.test.tsx',
  "redo re-publishes the day": '≈ state/store.test.ts',
  "publish-day toggles off": '≈ ui/editweek.test.tsx',
  "no publish-all button on the edit page": '≈ ui/editweek.test.tsx (never rendered)',
  "the week banner carries no Publish AL button": '≈ ui/editweek.test.tsx',
  "the day that carries the edits carries the button": 'ui/editweek.test.tsx',
  "and a day with nothing pending carries none": 'ui/editweek.test.tsx',
  "the view page never renders one": '≈ ui/editweek.test.tsx',
  "publishing an AL needs that day signed": '≈ publish.test.ts + ui/editweek.test.tsx',
  "signing that day again unlocks it": 'ui/editweek.test.tsx',
  "AL1 published": 'ui/editweek.test.tsx',
  "only changed item marked AL1": '≈ ui/editweek.test.tsx',
  "AL panel lists AL1": 'ui/editweek.test.tsx',
  "AL has unpublish control": 'ui/editweek.test.tsx',
  "unpublish clears AL1 marks": 'ui/editweek.test.tsx',
  "undo+redo buttons": 'ui/editweek.test.tsx',
  "undo restores slot": '≈ state/store.test.ts + ui/editweek.test.tsx',
  "redo reapplies slot": '≈ state/store.test.ts + ui/editweek.test.tsx',
  "right-click clears slot": 'ui/editweek.test.tsx',
  "#17 right-click obeys the Edit-mode switch": '≈ ui/editweek.test.tsx (as behaviour)',
  "the palette header says what is being planned": 'ui/editweek.test.tsx',
  "a darkened name cannot be planted": '≈ state/store.test.ts + ui/editweek.test.tsx',
  "a name still showing can": '≈ ui/editweek.test.tsx',
  "and the slot stays armed after a refusal": '≈ ui/editweek.test.tsx',
  "the ineligible are darkened, not dropped from the list": '≈ ui/editweek.test.tsx (.rpuck.no rendered)',
  "the week arms only EMPTY positions": '≈ ui/editweek.test.tsx (via routeClick gate)',
  "arming and planting exist instead": '≈ state/store.test.ts',
  "view page stays read-only": 'ui/editweek.test.tsx',
  "edit roster pilots": '≈ ui/editweek.test.tsx (palette renders)',
  "edit roster SANS group": '≈ ui/editweek.test.tsx (SANS column in paletteHTML)',
  ...
  "cxText falls back to a plain CX with no reason": 'board.test.tsx',
  // —— the scheduler board (phase 4h) ——
  "sched board opens": 'board.test.tsx',
  "sched lines render": 'board.test.tsx',
  "sched roster render": 'board.test.tsx',
  "sched wave title select": 'board.test.tsx',
  "sched setSlotVal": 'board.test.tsx',
  "the board shows the open day's strip": 'board.test.tsx',
  "board inputs panel": 'board.test.tsx',
  "board inputs banded": 'board.test.tsx',
  "the CX button opens a box, it does not toggle silently": 'board.test.tsx',
  "it asks for a reason under a CX DUE label": 'board.test.tsx',
  "nothing is cancelled just by opening it": 'board.test.tsx',
  "it offers the usual reasons": 'board.test.tsx',
  "Un-cancel is hidden on a line that is not cancelled": 'board.test.tsx',
  "the action button says what it will do": 'board.test.tsx',
  "a quick reason fills the field": 'board.test.tsx',
  "saving cancels the line and keeps the reason": 'board.test.tsx',
  "the line reads CX DUE <reason> everywhere it is drawn": 'board.test.tsx',
  "re-clicking CX reopens the box to edit the reason": 'board.test.tsx',
  "Un-cancel restores the line and drops the reason": 'board.test.tsx',
  "the change is recorded for the amendment": '≈ board.test.tsx (a board field commit earns its pending mark)',
  "logout closes the scheduler board": 'board.test.tsx',
  // —— drag & drop (phase 4i) ——
  "duty cells droppable": 'drag.test.tsx',
  "sim cells droppable": 'drag.test.tsx',
  "ground cells droppable": 'drag.test.tsx',
  "programme cells droppable": 'drag.test.tsx',
  "available crew are drag sources": 'drag.test.tsx',
  "roster -> duty seat": 'drag.test.tsx',
  "flying <-> duty swap": 'drag.test.tsx',
  "roster -> programme append": 'drag.test.tsx',
  "roster -> empty sim front seat": 'drag.test.tsx',
  "seat -> roster unassigns": 'drag.test.tsx',
  "dnd class cleared after drop": 'drag.test.tsx',
  "view page stays read-only": 'drag.test.tsx',
  "dragFrom / applyDrop are exposed": '≈ drag.test.tsx (module exports, not window globals)',
  "dragFrom reads a slot seat": 'drag.test.tsx',
  "dragFrom reads a roster puck": 'drag.test.tsx',
  "dragFrom ignores anything undraggable": 'drag.test.tsx',
  "applyDrop is a no-op without a drag": 'drag.test.tsx',
  "the mouse drop handler delegates to applyDrop": '≈ drag.ts (verbatim body; behaviour covered by the dnd tests)',
  "the swap zone has a margin": '≈ drag.ts (verbatim nearSeat + SWAP_SLOP; jsdom has no geometry to measure)',
  "a near miss is resolved to the nearest puck": '≈ drag.ts (verbatim body; probe re-measures in phase 5)',
  "below a puck adds rather than resolving back to it": '≈ drag.ts (verbatim body; probe re-measures in phase 5)',
  "and dropping a puck back where it started says so": 'drag.test.tsx',
  "both input paths hand the pointer to applyDrop": '≈ drag.ts (mouse drop and touch lift both call applyDrop)',
  "a jet line refuses a third body": 'drag.test.tsx',
  "every list cell is an append target": 'drag.test.tsx',
  "touch runs the same applyDrop": '≈ drag.ts (verbatim touch state machine)',
  "touch drag is wired to pointer events": '≈ drag.ts (verbatim; pointerdown/move/up/cancel attached in initDrag)',
  "mouse pointers are left to native HTML5 dnd": '≈ drag.ts (verbatim pointerType===\'mouse\' bail)',
  "a press-and-hold arms the drag, a quick move does not": '≈ drag.ts (verbatim TD_HOLD/TD_SLOP machine)',
