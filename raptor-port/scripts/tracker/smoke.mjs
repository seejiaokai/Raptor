/*
 * Browser smoke suite for the OCU Progress Tracker — VENDORED into Raptor
 * (7 Sep 26) from github.com/seejiaokai/Tracker scripts/smoke.mjs, where it
 ...
 *   APP_URL=http://localhost:4173/ npm run smoke:tracker     an already-running preview
 *
 * Chromium: the dev container's build at /opt/pw-browsers/chromium when
...
pg.on('response', r => { if (r.status() >= 400) bad4xx.push(r.status() + ' ' + r.url()); });
...
/* Show All is a plain button in the bar again (2 Sep): the View menu that held
   it had one other item, Reorder crew, which now lives in the Students card. */
...
{
  const EO = await import('../../src/tracker/app/eventOrder.js');
...
/* Reorder crew is about the students, so it sits with + Add in the Students
   card — not in a "View" menu two clicks away from the people it moves. */
ok('Reorder crew sits in the Students card beside + Add', await pg.evaluate(() => {
  const b = document.getElementById('ordCrew');
  return !!b && !!b.closest('.c-students') && b.getBoundingClientRect().width > 0
    && !!document.getElementById('addStu').closest('.c-students');
}));
...
{
  await openShowAll(); await pg.waitForTimeout(500);
   ...
   than compared against "whatever was there before". */
{
  ...
  if (roster0.length >= 2) {
    ...
    await pg.click('#ordCrew'); await pg.waitForSelector('#ordModal');
...
/* ---- the edit-mode hint must not push the chart about, nor sit on the legend ----
   ...
   spot. Its wrapper is therefore a FIXED one line while editing: switching
...
const hintGeom = () => pg.evaluate(() => {
  ...
  return {
    ...
    legendClear: lr.top >= hr.bottom - 1,
  };
});
const moveState = await hintGeom();
await pg.click('#arrTools button:has-text("╱ Line")'); await pg.waitForTimeout(250);
const lineState = await hintGeom();
ok('switching to the Line tool does not move the board',
  lineState.top === moveState.top && lineState.wrapHeight === moveState.wrapHeight,
  `top ${moveState.top} -> ${lineState.top}, wrapper ${moveState.wrapHeight} -> ${lineState.wrapHeight}`);
ok('the hint is visible, takes exactly one line of real space, and passes clicks through',
  lineState.hintShown && lineState.hintHeight > 10 && lineState.passesClicks
    && moveState.wrapHeight > 20 && Math.abs(moveState.wrapHeight - moveState.hintHeight) < 2,
  JSON.stringify(lineState));
/* Move's hint is the resting state and fits the line; the Line tool's hint is
   an essay that wraps and spills below on purpose, for as long as that tool is
   selected. Only the resting state is pinned. */
...
const { SYLLABI, DEFAULT_SYL_ORDER } = await import('../../src/tracker/data/syllabi.js');
...
const { DEFAULT_LAYOUTS: LAYOUTS_FOR_LINES } = await import('../../src/tracker/data/layouts.js');
const stray = [];
...
const { EVENT_INFO, EVENT_INFO_BY_SYL } = await import('../../src/tracker/data/eventInfo.js');
...
{
  const { DEFAULT_LAYOUTS: DL } = await import('../../src/tracker/data/layouts.js');
...
const { DEFAULT_LAYOUTS } = await import('../../src/tracker/data/layouts.js');
...
const { SEED_STATE } = await import('../../src/tracker/data/seedState.js');
...
const rosterLeaks = [];
for (const f of ['src/tracker/app/core.js', 'sample-data/OCU_state_sample.json']) {
...
/* ---- 2 Sep review: Escape everywhere, course switching, the hints, the phone ---- */
await pg.setViewportSize({ width: 1500, height: 950 });
await openTracker(pg); await pg.waitForSelector('#flowSvg .ball'); await pg.waitForTimeout(300);
{
  /* Escape closed five things and ignored four: the poke-ball editor, the event
     details editor, the reorder list and every confirm / prompt box. One key,
     one contract — and a confirm answered with Escape closes ONLY itself, never
     the editor that asked. That editor also used to shut itself BEFORE asking
     "Delete this ball?", so a "no" left nothing on screen. */
  ...
  await pg.keyboard.press('Escape'); await pg.waitForTimeout(200);
  ok('Escape answers a confirm box with "no"', await pg.locator('#dlgModal').count() === 0);
  ok('…leaves the editor that asked still open', await pg.locator('#editModal').count() === 1);
  ok('…and deletes nothing', await pg.locator('#flowSvg .ball[data-id="ST-01"]').count() === 1);
  await pg.keyboard.press('Escape'); await pg.waitForTimeout(200);
  ok('Escape closes the poke-ball editor', await pg.locator('#editModal').count() === 0);
  ...
  await pg.click('#popEditInfo'); await pg.waitForSelector('#infoModal');
  await pg.keyboard.press('Escape'); await pg.waitForTimeout(200);
  ok('Escape closes the event details editor', await pg.locator('#infoModal').count() === 0);
  if (await pg.locator('#ifCancel').count()) await pg.click('#ifCancel');

  await viaMenu('course', '#ordCourse'); await pg.waitForSelector('#ordModal');
  await pg.keyboard.press('Escape'); await pg.waitForTimeout(200);
  ok('Escape closes the reorder list', await pg.locator('#ordModal').count() === 0);
  if (await pg.locator('#ordCancel').count()) await pg.click('#ordCancel');
}
{
  /* Switching COURSE with unsaved flow edits threw them away without a word,
     and left the status saying "unsaved flow edits" about the fresh chart.
     Switching syllabus has always asked; now both go through the same door. */
  const opts = () => pg.evaluate(() => [...document.querySelectorAll('#courseSel option')].map(o => o.value));
  if ((await opts()).length < 2) {
    await viaMenu('course', '#addCourse'); await pg.waitForSelector('#dlgInput');
    await pg.fill('#dlgInput', 'SMOKE OTHER'); await pg.click('#dlgOk'); await pg.waitForTimeout(600);
  }
  const from = await pg.inputValue('#courseSel');
  ...
  if (await pg.locator('#dlgCancel').count()) await pg.click('#dlgCancel'); await pg.waitForTimeout(400);
  ok('saying no keeps the course and the edits', await pg.inputValue('#courseSel') === from
    && await pg.locator('#flowSvg .ball[data-id="SMOKE TMP"]').count() === 1, await pg.inputValue('#courseSel'));
  await pg.selectOption('#courseSel', to); await pg.waitForTimeout(400);
  if (await pg.locator('#dlgOk').count()) await pg.click('#dlgOk'); await pg.waitForTimeout(700);
  ok('saying yes switches course and stops claiming there are unsaved flow edits',
    await pg.inputValue('#courseSel') === to && !/unsaved/.test(await pg.textContent('#saveStat')),
    `${await pg.inputValue('#courseSel')} — ${await pg.textContent('#saveStat')}`);
  await pg.selectOption('#courseSel', from); await pg.waitForTimeout(600);
}
{
  /* On a phone the edit-mode and Details-mode hints lay over the Flow chart /
     Info / Show All tabs (the legend they cover on a desktop is hidden here),
     and "reset" on the zoom control meant a literal 100% — 912px of chart on a
     390px screen — rather than the fitted width the chart opens at. */
  ...
  const tabClear = () => pp.evaluate(() => {
    const t = document.querySelector('#viewtabs button[data-view=info]'); const r = t.getBoundingClientRect();
    const hints = [...document.querySelectorAll('.arrhint.on')].map(h => h.getBoundingClientRect().bottom);
    return { tabTop: Math.round(r.top), hintBottom: Math.round(Math.max(0, ...hints)),
      hit: document.elementFromPoint(r.left + r.width / 2, r.top + r.height / 2) === t };
  });
  await pp.tap('#detailsBtn'); await pp.waitForTimeout(300);
  const d = await tabClear();
  ...
  const z0 = await zr();
  await pp.tap('#fzIn'); await pp.waitForTimeout(200); const z1 = await zr();
  await pp.tap('#fzReset'); await pp.waitForTimeout(200); const z2 = await zr();
  ...
  const ttl = await pp.evaluate(() => { const h = document.querySelector('#showAllPanel .sahd h2');
    return { h: Math.round(h.getBoundingClientRect().height), text: h.innerText.trim() }; });
  ok('phone: the Show All title is one short line', ttl.h < 30 && ttl.text === 'All events', JSON.stringify(ttl));
  await pp.close();
}
   ...
   ten-point polygon); the type is not written into the page anywhere else. */
{
  ...
  await grade(flights[0], '2026-08-20', 'DCO');
  ok('marking a flight done sets Last Flown to its date', await flown() === '2026-08-20 / 2026-08-20', await flown());
  await grade(flights[1], '2026-08-05', 'DCO');
  ok('an older flight marked afterwards does not drag Last Flown back', await flown() === '2026-08-20 / 2026-08-20', await flown());
  await grade(flights[1], '2026-08-25', 'DCO');
  ok('a later flight moves Last Flown forward', await flown() === '2026-08-25 / 2026-08-25', await flown());
  for (const id of flights) { await clickBall(id); await pg.waitForSelector('#pop'); await pg.locator('#pop button', { hasText: 'Not done' }).click(); await pg.waitForTimeout(300); }
}
/* ---- the Font box shows the size the chart really uses ----
   It was hard-wired to 8.5, whatever the chart had been set to. A reload in
...
/* ---- the side panel's event chips: click to jump, hover / long-press to read ----
   The user, 3 Sep: clicking ST-01 under Plannable now snaps the chart to that
   ball whatever the left side is showing; hovering shows name, type, hours,
   crew and prerequisites; on a phone a long press shows the same bubble. */
{
  await openTracker(pg); await pg.waitForSelector('#flowSvg .ball'); await pg.waitForTimeout(300);
  if (!(await pg.locator('.c-plan .chip').count())) { await addStudent('STUDENT CHIP'); await pg.waitForTimeout(300); }
  const where = () => pg.evaluate(() => {
    ...
    return { id, found: true, inView: r.top >= bd.top && r.bottom <= bd.bottom, ring: !!g.querySelector('circle.found') };
  });
  const bubble = () => pg.evaluate(() => { const d = document.getElementById('detailBubble'); return d && d.style.display !== 'none' ? d.innerText : ''; });
  /* the Next-event chip is the one furthest down the chart, so the jump is a real scroll */
  const chip = pg.locator('.c-next .branch').last(); const id = (await chip.textContent()).trim();
  await pg.evaluate(() => { document.getElementById('board').scrollTop = 0; });
  await chip.hover(); await pg.waitForTimeout(250);
  const hov = await bubble();
  ok('hovering a side-panel chip shows the event details bubble', hov.startsWith(id) && /Type:/.test(hov), JSON.stringify(hov));
  await chip.click(); await pg.waitForTimeout(600);
  const w = await where();
  ok('clicking a side-panel chip snaps the chart to that ball and rings it', w.id === id && w.found && w.inView && w.ring, JSON.stringify(w));
  ok('the bubble goes when the chip is clicked', (await bubble()) === '');
  await pg.mouse.move(5, 5);

  const pp = await b.newPage({ viewport: { width: 390, height: 844 }, hasTouch: true, isMobile: true });
  await openTracker(pp); await pp.waitForSelector('#flowSvg .ball'); await pp.waitForTimeout(400);
  await pp.tap('#viewtabs button[data-view=info]'); await pp.waitForTimeout(300);
  const pc = pp.locator('.c-next .branch').last(); const pid = (await pc.textContent()).trim();
  const bb = await pc.boundingBox();
   ...
   touch input. */
{
  ...
  const same = (a, z) => ['balls', 'lines', 'undo', 'redo', 'save', 'lay'].filter(k => a[k] !== z[k]);
