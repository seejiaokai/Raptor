...
const R = resultBook('W5-05d', `${ROOT}/docs/handpass/parts/2026-09-26-absence-w5-05d.txt`)
...
async function world(tag, trigger) {
  ...
  await shot(page, `w5-05d-${tag}`)
  if (tag.startsWith('V4')) { const rl = await X.reload(page, 'a'); await lwOpen(page, '2026-07-17'); const r2 = await rowRun(page, 'slipway', JUL); await shot(page, `w5-05d-${tag}-after-reload`); R.note(tag + '-before-reload', r); r = r2 }
