...
async function selectOneDay() {
  ...
  if (PHONE) {
    ...
    for (let i = 1; i <= 6; i++) { await cdp.send('Input.dispatchTouchEvent', { type: 'touchMove', touchPoints: [{ x: c.x + (c2.x - c.x) * i / 6, y: c.y, id: 3 }] }); await page.waitForTimeout(30) }
...
await step('build', async () => {
  ...
  R.ck('G2-3-pm-bid', s.open === 'select-sheet' && f.pressed && r3.cell.mark === '+2' && r3.agree, 'a one-day hold/drag opens the selection sheet; OL afternoon fills beside them (+2)', { open: s.open, span: (s.text || '').slice(0, 60), cell: r3.cell })
  /* 4–8 — the Inputs page */
  const inputs = [
...
await step('list', async () => {
  ...
  R.ck('G2-money', top(r.sheet.lve) === top(r0.sheet.lve) - 0.5 - (PHONE ? 0.5 : 0) && top(r.sheet.oil) === top(r0.sheet.oil) + 1 && top(r.sheet.medtot) === top(r0.sheet.medtot) + 0.5 && r.agree,
    `money: LVE −0.5 on the day (two equal 1-hour leaves — the earlier LL pays the morning, OL nothing; the bids are gone)${PHONE ? ' and −0.5 for the 7th afternoon bid' : ''}, OIL +1 (the award), MED TOT +0.5`, { lve: [r0.sheet.lve, r.sheet.lve], oil: [r0.sheet.oil, r.sheet.oil], medtot: [r0.sheet.medtot, r.sheet.medtot], bd: r.breakdown, disagree: r.disagree })
