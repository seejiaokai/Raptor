/* W4 — the published SATURDAY (26 Sep 26): finding AB6 (Fable S6 / F6, predicted) and SANS (Fable S39, Astra 22).
   ...
   RE-WALK COPY (26 Sep 26, re-walker W4): pictures to rewalk/w4, results to the rewalk-w4-saturday file. The AB6 checks
   now read the fix's own words (register §12): the tail line "LL — local leave, from the day before till 06:00" with
   "It runs on from the day before — change it there." under it, no buttons on it — and the member is read at BOTH
   widths (the first walk read him at 390 px only). */
...
const R = resultBook('W4-sat', `${ROOT}/docs/handpass/parts/2026-09-26-absence-rewalk-w4-saturday.txt`)
...
const STATEFILE = `${SP}/rw-w4-saturday-state.json`
let page
...
/* the tail line(s) of the open tap list, read as drawn: main text, the line under it, its class and any buttons on it */
const tailRead = pg => pg.evaluate(() => {
  const s = [...document.querySelectorAll('.bidsheet[role="dialog"]')].filter(e => e.offsetWidth || e.offsetHeight).pop()
  if (!s) return { open: 'nothing', tails: [] }
  const clash = s.querySelector('[data-testid="daylist-clash"]')
  return {
    open: s.getAttribute('data-testid'),
    clash: clash ? clash.innerText.trim() : '',
    tails: [...s.querySelectorAll('[data-testid^="dl-t-"]')].map(li => ({
      main: ((li.querySelector('.dl-main') || {}).innerText || '').trim(), sub: ((li.querySelector('.dl-sub') || {}).innerText || '').trim(),
      cls: String(li.className), buttons: li.querySelectorAll('button').length,
      color: getComputedStyle(li).color, bg: getComputedStyle(li).backgroundColor, border: getComputedStyle(li).borderLeftColor })),
    others: [...s.querySelectorAll('[data-testid="daylist"] li')].filter(li => !/^dl-t-/.test(li.getAttribute('data-testid') || '')).map(li => (li.innerText || '').replace(/\s+/g, ' ').trim().slice(0, 110)),
  }
})
const TAIL_MAIN = /^LL — local leave, from the day before till 06:00$/
const TAIL_SUB = /^It runs on from the day before — change it there\.$/
const tailOk = t => t.open === 'daylist-sheet' && t.tails.length === 1 && TAIL_MAIN.test(t.tails[0].main) && TAIL_SUB.test(t.tails[0].sub) && t.tails[0].buttons === 0 && t.others.some(l => /OIL earned|^FO/.test(l))
...
/* ============================================================= BUILD (desktop, admin) */
{
  ...
  await step('AB6-desktop', async () => {
    ...
    R.ck('AB6-list-shows-what-clashes', tailOk(tr) && /can’t both stand/.test(tr.clash), 'register §12: the admin\'s clash sentence, the credit, AND the tail named — "LL — local leave, from the day before till 06:00" / "It runs on from the day before — change it there." — with no buttons on the tail line', tr)
    ...
    await closeSheets(page)
    R.ck('AB6-money', /FO/.test(sat.box) && /LL>/.test(fri.box) && top(figs.sheet.lve) === -0.5 && top(figs.sheet.oil) === 1 && figs.agree,'money: the Saturday credit lands (FO), the leave charges Friday only; every reader agrees', { sat: sat.box, fri: fri.box, lve: figs.sheet.lve, oil: figs.sheet.oil, disagree: figs.disagree })
...
/* ============================================================= READ at 390 px (the same world), admin then member */
{
  ...
  await step('AB6-phone-admin', async () => {
    ...
    R.ck('AB6-phone-list', sat.amber && tailOk(tr) && /can’t both stand/.test(tr.clash), 'at 390 px, admin: amber, the clash sentence, the credit and the tail line in the fix\'s words, no buttons on it', { sat, tr, open: t.open })
    R.note('AB6-phone-admin-geometry', await page.evaluate(() => { const li = document.querySelector('[data-testid^="dl-t-"]'); if (!li) return null; const b = li.getBoundingClientRect(); return { left: Math.round(b.left), right: Math.round(b.right), top: Math.round(b.top), bottom: Math.round(b.bottom), vw: innerWidth, vh: innerHeight, overX: document.documentElement.scrollWidth - innerWidth } }))
    await closeSheets(page)
    const r = await readAll(page, sans, WED, { bd: ['lve'] })
  ...
  await step('AB6-member', async () => {
    ...
    R.ck('AB6-member-reads-the-tail', sat.amber && tailOk(tr) && /cover the same time/.test(tr.clash), 'Ranger himself, on his own Saturday at 390 px: the member\'s own clash sentence, then the credit AND his leave\'s tail in the fix\'s words, no buttons on it', { sat, tr, open: t.open })
    await closeSheets(page)
...
/* ============================================================= RE-WALK ADDITION: the member at the DESKTOP width */
{
  const o = await openW4({ phone: false, who: 'a', state: STATEFILE })
  page = o.page
  await step('AB6-member-desktop', async () => {
    await relogin(page, 'm')
    await lwOpen(page, SAT)
    const sat = await cellOf(page, me, SAT)
    const t = await tapDay(page, false, me, SAT)
    const tr = await tailRead(page)
    await shot(page, 'w4-sat-AB6-desktop-member-sat-list')
    R.ck('AB6-member-desktop-reads-the-tail', sat.amber && tailOk(tr) && /cover the same time/.test(tr.clash), 'Ranger himself, desktop: the member\'s clash sentence, the credit and his leave\'s tail in the fix\'s words, no buttons', { sat, tr, open: t.open })
    await closeSheets(page)
    const tf = await tapDay(page, false, me, FRI)
    await shot(page, 'w4-sat-AB6-desktop-member-fri')
    R.note('AB6-member-friday-sheet', { open: tf.open, text: (tf.text || '').slice(0, 300), lines: lines(tf) })
    await closeSheets(page)
    const own = await readAll(page, me, SAT, { bd: ['lve'] })
    R.ck('member-desktop-own-figures', own.agree && top(own.sheet.oil) === 1 && top(own.sheet.lve) === -0.5, 'money unchanged for the member at the desktop: OIL 1 (Saturday\'s credit), LVE −0.5 (Friday\'s afternoon only), every reader agreeing', { sheet: own.sheet, disagree: own.disagree })
  })
  R.note('desktop-member-errors', o.errors.slice(0, 20))
  await o.browser.close()
}
R.save()
