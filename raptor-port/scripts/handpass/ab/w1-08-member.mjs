...
const R = resultBook('W1-08-member', `${ROOT}/docs/handpass/parts/2026-09-26-absence-w1-08-member.txt`)
...
await step('OTHER', async () => {
  ...
  R.ck('OTHER-not-moved', days.join() === '2026-07-24' && t.some(x => /someone else/i.test(x)), `${who}'s chip stays on 24 Jul; the member is told why`, { days, t })
