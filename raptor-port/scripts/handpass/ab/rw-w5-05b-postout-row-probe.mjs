/* W5 RE-WALK copy (26 Sep 26) for W5-F1: two checks added — Drifter's and Cobra's place on the Quals page (roster, not
   Archived) after the posting sheet's "Undo post out".
...
const PHONE = W === 'phone'
const R = resultBook(`RW-W5-05b-${W}`, `${ROOT}/docs/handpass/parts/2026-09-26-absence-rewalk-w5-05b-${W}.txt`)
const { browser, page, errors } = await openHi({ width: PHONE ? 390 : 1440, height: PHONE ? 844 : 900, who: 'a', dpr: PHONE ? 2 : 1 })
await toastSpy(page)
const pic = n => `rw-w5-05b-${W}-${n}`
...
const q3 = await Q.quals(page, D, pic('3b-quals-drifter-after-undo'))
R.ck('3-undo-postout-restores', u.pressed && !/NO CELL/.test(after3.jul14) && /LL/.test(after3.jul17) && !/PO/.test(after3.jul17) && q3.onRoster && !q3.inArchive,
  '"Undo post out (PO)" on Drifter (archive on): his row back in June and July, 17 Jul LL without the PO tag, and he is on the Quals roster (W5-F1)', { after3, q3 })
...
const q4 = await Q.quals(page, C, pic('4b-quals-cobra-roster'))
R.ck('4-cobra-quals-roster', q4.onRoster && !q4.inArchive, 'after "Undo post out", Cobra is on the Quals roster, not in its Archived section (W5-F1)', q4)
