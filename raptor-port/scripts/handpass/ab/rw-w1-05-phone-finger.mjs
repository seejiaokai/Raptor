/* RE-WALK copy of w1-05-phone-finger.mjs (26 Sep 26, the absence-record re-walk, W1): pictures go to docs/img/handpass/2026-09-26-absence/rewalk/w1/, results to docs/handpass/parts/2026-09-26-absence-rewalk-w1-*.txt; every change of premise or added check is marked RE-WALK. The first walk's script is untouched. */
...
const R = resultBook('W1-05-phone', `${ROOT}/docs/handpass/parts/2026-09-26-absence-rewalk-w1-05-phone.txt`)
...
/* ---- RE-WALK, keep an eye out: W1-F1's fix lets the browser take VERTICAL finger movement on the grid (it pans the
   month up and down). A held chip carried DOWN a row and back UP must still land (the drag's own touchmove holds the
   page still once armed); a vertical finger sweep on empty space scrolls the grid and neither pages nor adds. ---- */
await step('DRAG-DOWN-UP', async () => {
  await calOpen(page, '2026-07'); await toasts(page)
  const g = await fingerDragChip(page, cdp, A.piston, '2026-07-20', '2026-07-28', { mid: 'rw-w1-05-drag-down-in-flight' })
  const t = await toasts(page)
  const d1 = await chipDays(page, A.piston)
  await shot(page, 'rw-w1-05-drag-down-after')
  R.ck('DRAG-DOWN', d1.join() === '2026-07-28' && noGhost(g.after), 'a finger carries Piston\'s LL DOWN a row (20 → 28 Jul): it lands, no ghost left', { d1, g, t })
  await calOpen(page, '2026-07'); await toasts(page)
  const g2 = await fingerDragChip(page, cdp, A.piston, '2026-07-28', '2026-07-20')
  const d2 = await chipDays(page, A.piston)
  R.ck('DRAG-UP', d2.join() === '2026-07-20' && noGhost(g2.after), 'and back UP a row (28 → 20 Jul): it lands', { d2, g2, t: await toasts(page) })
})
await step('VSCROLL', async () => {
  await calOpen(page, '2026-07')
  const st0 = await page.evaluate(() => document.querySelector('#inpCal .ic-grid').scrollTop)
  const sh = await page.evaluate(() => { const g = document.querySelector('#inpCal .ic-grid'); return { sh: g.scrollHeight, ch: g.clientHeight, ta: getComputedStyle(g).touchAction } })
  const p = await emptyAt(page, '2026-07-22')
  const n0 = await page.evaluate(() => window.INPUTS.length)
  await finger(page, cdp, p, { x: p.x + 4, y: Math.max(120, p.y - 260) }, { steps: 10, stepMs: 16 })
  const st1 = await page.evaluate(() => document.querySelector('#inpCal .ic-grid').scrollTop)
  const m = await calMonth(page), pop = await popover(page), dlg = await addDialog(page)
  const n1 = await page.evaluate(() => window.INPUTS.length)
  await shot(page, 'rw-w1-05-vscroll-after')
  R.ck('VSCROLL', m === 'July 2026' && !pop.open && !dlg.open && n0 === n1, 'a vertical finger sweep on empty space neither pages the month nor opens / adds anything', { m, pop: pop.open, dlg: dlg.open, n0, n1 })
  R.note('VSCROLL-scrolled', { st0, st1, ...sh, why: 'pan-y: the grid scrolls when it is taller than the screen' })
})
...
await step('FLICK-CHIP', async () => {
  ...
  /* RE-WALK (new evidence): the first run of this copy found the chip's EDIT open after the flick (the first walk's
     browser cancelled a sideways finger, so it could not happen then). A flick is not a tap: nothing should open.
     Closed here so the steps after it start clean. */
  const dF = await addDialog(page)
  await shot(page, 'rw-w1-05-flick-chip-after')
  R.ck('FLICK-CHIP-no-edit', !dF.open, 'a sideways flick that starts on a chip opens nothing (it is not a tap)', dF)
  await closeDialog()
})
