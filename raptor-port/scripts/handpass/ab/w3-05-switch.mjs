...
const PHONE = W === 'phone'
const R = resultBook(`W3-05-${W}`, `${ROOT}/docs/handpass/parts/2026-09-26-absence-w3-05-${W}.txt`)
...
for (const [name, openIt, press] of SHEETS) {
  await step(`E1-${name}`, async () => {
    ...
    R.ck(`E1-${name}`, sw.now === B && (after.open === 'nothing' || !press || !pressed || pressed.open === after.open) && !/request|credit/.test(recs27) && reopened.open === 'nothing',
      `with the ${name} open, switching to JAN–DEC 27 closes it (or leaves it inert), nothing is written into 27, and coming back to 26 does not re-open a stale sheet`,
