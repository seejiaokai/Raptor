/* W5 RE-WALK (26 Sep 26) — CANARIES in rw-w5-05's world: the published-Friday pair's rows vanished there (F2's symptom)
   though every single-prefix world of rw-w5-05e kept them. At each point of the same flow, one more man (a canary,
   not one of the Friday pair) is posted out from 15 Jul through the bid sheet; is his July row still drawn? The first
   canary that vanishes names the step. The Friday pair is fixed to the men rw-w5-05 picked (Drifter, Ace).
   (A copy of rw-w5-05-postout-file.mjs; its own checks unchanged.)
...
const PHONE = W === 'phone'
const R = resultBook(`RW-W5-05f-${W}`, `${ROOT}/docs/handpass/parts/2026-09-26-absence-rewalk-w5-05f-${W}.txt`)
const { browser, page, errors } = await openHi({ width: PHONE ? 390 : 1440, height: PHONE ? 844 : 900, who: 'a', dpr: PHONE ? 2 : 1 })
await toastSpy(page)
async function step(name, fn) { try { await fn() } catch (e) { R.ck(name, false, 'step ran', 'THREW ' + String(e && e.message || e).slice(0, 300)); await shot(page, `rw-w5-05-${W}-THREW-${name}`).catch(() => {}) } }
const pic = n => `rw-w5-05f-${W}-${n}`
const SEP = ['2026-09-10', '2026-09-11', '2026-09-14']
const CAN = ['dice', 'pump', 'snap', 'bapster', 'haowen', 'shrek', 'prowler', 'anvil']
let ci = 0
async function canary(where) {
  const id = CAN[ci++]
  try {
    const po = await Q.postOutBid(page, id, '2026-07-28', '2026-08-01', true)
    await lwOpen(page, '2026-07-14')
    const r = await L.rowRun(page, id, ['2026-07-13', '2026-07-14'])
    const st = await page.evaluate(p => { const x = window.PEOPLE[p] || {}; return { cs: x.cs, archived: !!x.archived, by: x.archivedBy || '' } }, id)
    if (r.some(x => /NO CELL/.test(x))) await lwShot(page, pic('CANARY-' + ci + '-' + where), 'taipan', '2026-07-14')
    R.ck('CANARY-' + ci + '-' + where, po.done && !r.some(x => /NO CELL/.test(x)), `canary ${id} posted out from 1 Aug after [${where}]: his July row still drawn`, { po: po.done, row: r, st })
  } catch (e) { R.note('CANARY-' + where + '-THREW', String(e.message || e).slice(0, 200)) }
}
const PO = '2026-08-01'
...
await canary('after-A-postout-then-file')
...
await canary('after-A-place-instead')
...
await step('B-file-then-postout', async () => {
  ...
  await canary('after-B-before-reload')
  const rl = await reload(page, 'a')
  await canary('after-reload')
  await lwOpen(page, SEP[0])
...
await canary('after-B-reload-snaps-quals')
