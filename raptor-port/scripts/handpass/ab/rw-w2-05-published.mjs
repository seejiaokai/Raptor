/* RE-WALK copy of w2-05-published.mjs on the REBUILT app (26 Sep 26): pictures to rewalk/w2, results to
   parts/2026-09-26-absence-rewalk-w2-05-*.txt; P6's last check walked with its new premise (W1-F3: another man's chip
   does not lift for a member). The published-day behaviour must be unchanged by the AB4 fix: a medical that meets no
   other medical still moves at once (P4).
...
const R = resultBook(`RW-W2-05-${WD}`, `${ROOT}/docs/handpass/parts/2026-09-26-absence-rewalk-w2-05-${WD}.txt`)
const { browser, page, errors } = await openHi({ width: PHONE ? 390 : 1440, height: PHONE ? 844 : 900, who: 'a', dpr: PHONE ? 3 : 1 })
await toastSpy(page)
const P = (n) => `rw-w2-05-${WD}-${n}`
...
await step('P6', async () => {
  ...
  await W.calTo(page, '2026-07')
  await toasts(page)
  /* RE-WALK premise (W1-F3 fixed, register §12: "A member's calendar does not lift another man's chip"): the first walk
     waited for the drop's refusal toast; now the chip must not lift at all — watched MID-gesture (a ghost, the
     dragging state, a lit day) — and nothing moves */
  const mid = await (async () => {
    const chip = page.locator(`#inpCal [data-icday="${JUL(14)}"] .ic-chip[data-iid="${grit}"]`).first()
    if (!(await chip.count())) return { why: 'no chip' }
    await chip.evaluate(e => e.scrollIntoView({ block: 'center' })); await page.waitForTimeout(250)
    const a = await chip.boundingBox(), b = await page.locator(`#inpCal [data-icday="${JUL(21)}"]`).first().boundingBox()
    const x0 = a.x + a.width / 2, y0 = a.y + a.height / 2, x1 = b.x + b.width / 2, y1 = b.y + Math.min(b.height - 6, 14)
    const read = () => page.evaluate(() => ({ ghosts: document.querySelectorAll('.ic-ghost').length, dragging: document.body.classList.contains('ic-dragging'), lit: document.querySelectorAll('#inpCal [data-icday].ic-over, #inpCal .ic-drop').length }))
    let during
    if (PHONE) {
      const cdp = await page.context().newCDPSession(page)
      const t = (type, touchPoints) => cdp.send('Input.dispatchTouchEvent', { type, touchPoints })
      await t('touchStart', [{ x: x0, y: y0, id: 1 }]); await page.waitForTimeout(320)
      for (let i = 1; i <= 8; i++) { await t('touchMove', [{ x: x0 + (x1 - x0) * i / 8, y: y0 + (y1 - y0) * i / 8, id: 1 }]); await page.waitForTimeout(20) }
      during = await read()
      await t('touchEnd', []); await page.waitForTimeout(700); await cdp.detach().catch(() => {})
    } else {
      await page.mouse.move(x0, y0); await page.mouse.down(); await page.mouse.move(x0 + 7, y0 + 3, { steps: 2 }); await page.mouse.move(x1, y1, { steps: 12 })
      await page.waitForTimeout(120); during = await read()
      await page.mouse.up(); await page.waitForTimeout(700)
    }
    return { during, after: await W.ghostLeft(page) }
  })()
  const t2 = await toasts(page)
  await shot(page, P('P6b-member-cannot-move-another'))
  R.ck('P6-member-not-another', !mid.during?.ghosts && !mid.during?.dragging && !t2.some(x => /Moved to/.test(x)) && (await W.medRows(page, 'sufa')).some(r => /^ATT C Jul 13–Jul 17/.test(r)),
    'Grit\'s chip does not lift for the member — no ghost, no dragging state mid-gesture — and it stays 13–17 (W1-F3, D166)', { mid, toasts: t2 })
})
