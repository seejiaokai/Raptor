/* RW-W3-01 — the RE-WALK copy of w3-01-bulk.mjs (26 Sep 26, the rebuilt app): pictures to rewalk/w3, results to
   2026-09-26-absence-rewalk-w3-01-<act>.txt; the checks follow the register §12 (AB7 "… already approved"; W3-F3 Delete takes the bid
   beneath filed leave and the filed leave stays; Move beneath is [LW-MOVE-BENEATH] — recorded, not judged; AB2 unchanged — confirmed).
...
const R = resultBook(`RW-W3-01-${ACT}`, `${ROOT}/docs/handpass/parts/2026-09-26-absence-rewalk-w3-01-${ACT}.txt`)
...
await step('act', async () => {
  ...
  if (ACT === 'delete') {
    ...
    R.note('delete-first-tap', { note: first.note }); globalThis.__delConfirm = first.note
    await shot(page, `w3-01-${ACT}-c-confirm`)
    R.note('AB2-confirm (unchanged on purpose — his question)', first.note)
...
if (ACT === 'delete') {
  R.ck('AB2-unchanged-confirmed', !/FO/.test(cell(after, 'spaceman', 2)) && !/FO/.test(cell(after, 'plasma', 0)),
    'AB2 is UNCHANGED on purpose (his question): bulk Delete still takes the two OIL awards — confirm only',
  ...
  R.note('AB2-confirm-text', 'the confirm said: ' + (globalThis.__delConfirm || '?'))
  ...
  R.ck('W3-F3-split-bid-deleted', !/request/.test(after.recs.riddler[D[0]]), 'the undecided afternoon bid that shares Ghost’s 16 Jul with his Inputs-filed morning is deleted too — it is an editable bid (the D1 shape, fixed for Approve)', { before: before.recs.riddler[D[0]], after: after.recs.riddler[D[0]], box: cell(after, 'riddler', 0) })
  R.ck('W3-F3-filed-leave-stays', after.inputs.riddler.some(x => /^LL Jul 16/.test(x) && /lw=n/.test(x)) && after.inputs.riddler.some(x => /^LL Jul 17/.test(x)) && /LL/.test(cell(after, 'riddler', 0)),
    'the leave Ghost filed on the Inputs page (16 Jul morning, 17 Jul) STAYS — it is the Inputs page’s (register §12)', { inputs: after.inputs.riddler, box16: cell(after, 'riddler', 0), box17: cell(after, 'riddler', 1) })
}
if (ACT === 'approve') {
  ...
  R.ck('AB7-count-true', c.written === 2 && /1 already approved/.test(res && res.note || ''), 'the note counts only what changed — "2 decided" — and says the already-approved leave apart: "1 already approved" (AB7, register §12)', { note: res && res.note, claimed: c, changed: ch.length, cells: ch })
}
...
if (ACT === 'move') {
  ...
  R.note('W3-F3-move-RECORDED ([LW-MOVE-BENEATH], filed — not judged)', { banner: res && res.note, riddler20: m20.riddler, riddler16: after.recs.riddler[D[0]], movedWithBlock: /LL/.test(m20.riddler.split(' | ')[0]) && !/request/.test(after.recs.riddler[D[0]]) })
}
if (ACT === 'moveclash') {
