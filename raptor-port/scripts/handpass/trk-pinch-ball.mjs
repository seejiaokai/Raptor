...
const look = page => page.evaluate(() => {
  ...
  const hint = document.querySelector('#page-tracker .arrhint')
  return {
...
const KEEP = ['balls', 'lines', 'undo', 'redo', 'save', 'lay', 'sel', 'band', 'overlay']
...
async function ensureView(page) {
  if (await ballNearMiddle(page)) return
  ...
  if (!edit || room < 150) {
       ...
       tap still does, where it lands (Astra 1) */
    {
      ...
      if (!p) note(`${label}: E8 Line — half-drawn line`, false, 'no bare canvas')
        ...
        await editCase(page, cdp, label, 'E8 Line — a line half-drawn by one tap: a pinch leaves it half-drawn', {
          keepView: true, at: async () => ({ x: p.x + 40, y: p.y + 60 }), keys: KEEP.filter(k => k !== 'overlay'), picture: 'E8-halfline',
        })
    ...
    if (!L1) note(`${label}: E9–E13 line cases`, false, 'could not draw a line to work on')
      ...
      if (L2) {
        ...
        await editCase(page, cdp, label, 'E13c Merge — line one armed by a tap, the first finger on line two: no merge', { keepView: true, at: () => lineAt(page, L2, 0.2) })
