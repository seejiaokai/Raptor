...
// Owner's rule, REWRITTEN 28 Aug 26: ONE law for every source — pool each
// person's worked minutes as an interval union (flying report→land+debrief,
...
import { dayOilCredits, dayOilSpans, mergeMin, uniformOil, inputOilAmt } from './oil'
...
describe('mergeMin / uniformOil / inputOilAmt — the shared arithmetic', () => {
  it('merges overlapping spans so no minute pays twice', () => {
    expect(mergeMin([[480, 720], [540, 600]])).toBe(240)            // 0800–1200 ∪ 0900–1000 = 4h
    expect(mergeMin([[480, 600], [600, 720]])).toBe(240)            // abutting spans chain
    expect(mergeMin([[480, 600], [720, 780]])).toBe(180)            // disjoint spans sum
    expect(mergeMin([])).toBe(0)
  })
...
describe('dayOilCredits — who earns what from one day blob', () => {
  ...
  it('overlapping rows pool as a UNION — the same hour never pays twice', () => {
    const d = day([], [duty('plasma', '0800', '1200')], { allhands: [
      { prog: 'Brief', str: '0900', end: '1000', who: 'plasma' },
    ] })
    expect(dayOilCredits(d)).toEqual({ plasma: 0.5 })   // 4h, not 5h
  })
  ...
  it('an ALL / ALL AVAIL sentinel on a programme or ground row expands via the resolver, or drops without one', () => {
    ...
    expect(credits).toEqual({ plasma: 0.5 })           // 4h + 1h pooled = 5h
  ...
  it('hours are pooled per person before the line is drawn', () => {
  ...
  it('SC plus a duty pools to one day — a day worked, not a day and a half', () => {
    ...
    expect(dayOilCredits(d)).toEqual({ plasma: 1 })   // 6h + 3h pooled = 9h
  })
