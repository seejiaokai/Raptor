...
import { SCHED, pendCount, alUsed, nextAL, canPublishAL, publishableKeys, alUnsignedDays, daysLabel, dowShort, dayApproved, dayPendCount, pendDays, alDays, alCount, publishAL, unpublishAL, discardPending } from '../engine/publish'
...
export function ALPanel() {
  ...
  const np = pendCount(), used = alUsed()
  const opts: number[] = []
  ...
  return (
    <div className="alpanel" id="alPanel">
      ...
      <div className="al-row">
        <span className={'al-pend' + (np ? ' on' : '')}>{np ? `${np} pending change${np > 1 ? 's' : ''}` : 'No pending changes'}</span>
      ...
      {SCHED.als.length
        ? <div className="al-list" dangerouslySetInnerHTML={{
          __html: SCHED.als.slice().sort((a: any, b: any) => a.n - b.n).map((a: any) =>
            `<span class="al-tag" data-alc="${a.n}" title="${a.sign ? alDays(a).map((di: number) => dowShort(di) + ': ' + SIGN_ROLES.map((r: any) => r[1] + ' ' + (((a.sign[di] || {})[r[0]]) || '—')).join(' · ')).join(' | ') : 'signed before sign-off was introduced'}"><b>AL${a.n}</b> <i class="al-days">${daysLabel(alDays(a))}</i> · ${alCount(a)} item${alCount(a) > 1 ? 's' : ''}${(() => { const s0 = a.sign && a.sign[alDays(a)[0]]; return s0 && s0.appr ? ` · <i class="al-days">appr ${esc(s0.appr)}</i>` : '' })()}<button class="al-un" data-alun="${a.n}" title="Unpublish AL${a.n}">✕</button></span>`).join('')
        }} onClick={e => {
