  ...
  {g:'How a day is measured',
   ...
   rows:[
    ...
    {sev:'set',set:['showLead'],src:()=>`VCONF.showLead ${VCONF.showLead}`,
     t:()=>`The <b>latest show</b> is ${lgV(lgT(VCONF.showLead))} before take-off.<span class="why">This is the line a <b>late show</b> cannot cross. A remark such as "2A: LATE SHOW" or "SHOW AT BRIEF" excuses that crew from the published in-time and from the brief, so crew rest running past brief time is no longer a breach — he joins the sortie late. Rest still running at the latest show is a different matter and stays a hard warning: he cannot walk, kit up and start engines in less than this, so he is not late to the brief, he is unable to make the flight.</span>`},
