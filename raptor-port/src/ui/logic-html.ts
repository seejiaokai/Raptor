  ...
  {g:'Leave, downchit and personal inputs',
   rows:[
    ...
    {sev:'adv',code:'FLT_NO_LEN',
     t:()=>`A flying line typed with the <b>same take-off and landing time</b> still earns the crew their OIL — but one of the two times is wrong, and the day says so. An Advisory, on the line, on <b>any</b> day.<span class="why">The sortie itself measures nothing, so on the face of it nobody should be paid for it. They are, and deliberately: a flying day runs from the report three hours before take-off to the debrief two hours after landing, and the crew did both. They were in squadron whatever the times say, which is the squadron's own fact rather than anything the times can show. So the line is never refused and no credit is taken away — the warning exists because a nought-minute sortie is always a slip of the keyboard, and it is cheaper to fix while the day is being built than after it has gone out. It names the line, not the crew: the times are the scheduler's to correct. A line that has no readable times at all is a different problem and carries the other warning — that one really does earn nobody anything.</span>`},
