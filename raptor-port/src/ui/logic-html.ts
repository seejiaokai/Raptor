...
export function lgRules(){
  ...
  {g:'Turning and the length of the day',
   rows:[
    ...
    {sev:'adv',code:'DT_SUM',
     t:()=>`<b>Double turning</b> — <b>two or more sorties in one day, flown one after the other</b> — raises one Advisory line at the head of the day naming everyone who is on it, matching the amber DT pucks. Double turning is routine and planned, not an error.<span class="why">There is no span test: a man on two sorties is double turning whether they are back to back or a whole day apart. How tight the gap is belongs to the tight-turn rule. And two seats on the same air time are not a double turn at all — that man is double-booked, the red conflict Warning speaks for him, and this line leaves him out.</span>`},
  ...
  {g:'Qualification and currency',
   rows:[
    {sev:'hard',code:'QUAL',
     t:()=>`<b>Seat rules.</b> A WSO cannot fly the front seat; <b>IW</b> is a WSO-only category and sits RCP only. The <b>rear seat is open to any pilot</b> — jet or sim, no instructor rating needed. A pilot <b>FI</b> follows the IP rules, a WSO FI the IW rules. The sim's front seat follows the same rule as the jet's.`},
    {sev:'hard',code:'ILLEGAL_CREW',
     t:()=>`<b>The combination matrix</b> (F-15 Table 1.5-2). An <b>OCU pilot</b> may not fly with a <b>CAT A–D WSO</b>, and an <b>OCU WSO</b> may not fly with a <b>CAT A–D pilot</b> — not authorised combinations, a Warning.<span class="why">An instructor in either seat clears the matrix: an instructor pilot (IP / IR / FI) flies with anyone, an instructor WSO with any front seat.</span>`},
    {sev:'adv',code:'CREW_SOLO',
     t:()=>`<b>OCU pilot with OCU WSO</b> is a <b>crew solo</b> — only allowed for sorties designated under the F-15 Basic Course Syllabus. An Advisory.`},
  ...
  {g:'Leave, downchit and personal inputs',
   rows:[
    ...
    {sev:'adv',code:'SANS_AVAIL',
     t:()=>`<b>SANS Availability</b> is the opposite of every type above — it says which events a SANS body <b>is</b> offering, Fly / AMT / OFT, and when. A SANS body planted into a flying seat, an OFT box or an AMT box <b>outside what a filed SANS Availability input offers</b> — the wrong event, or outside the stated time — raises an Advisory. It wears the amber <b>A</b> flag — the planned-against-what-was-filed chip (owner, 26 Aug 26; it launched on CP, but CP says a crew <i>pairing</i> needs approval, which is not this man's problem). A flying seat is judged from the crew's <b>in-time to landing + dekit</b> (owner, 26 Aug 26): the wave's published in-time — or a typed SC B — opens the window when it shows the crew earlier than the step; with no in-time published the window stays step to dekit, and a later in-time never narrows it. A sim box is judged at its own written hours.<span class="why">Filing <b>nothing</b> raises no Advisory here — only the palette's grey-out, printed the moment a slot is armed, and the same toast on planting anyway. This is deliberate: the demo week's SANS aircrew fly with no SANS Availability record filed at all, and turning a blank record into a persistent warning would flag every one of them for something nobody has asked them to file yet.</span>`},
    ...
    {sev:'note',set:['oilFullMin'],src:()=>`VCONF.oilFullMin ${VCONF.oilFullMin} · engine/oil.ts`,
     t:()=>`<b>Work on a weekend or public holiday earns OIL</b>, credited automatically into Leave War when the day is <b>published</b>. ONE rule for everything: a person's worked hours for the day run START TO FINISH — from the first thing they do to the end of the last, gaps between events included, because they are still in squadron — over flying (report to landing plus debrief), sims, duty rows, the Ground and Common Programme and SC MAIN shifts. <b>${lgV(ruleFmt('oilFullMin',VCONF.oilFullMin))}</b> or more is a FULL day (FO, 1.0), under it a HALF (HO, 0.5), regardless of the time of day or the shift's shape.<span class="why">A public holiday is whatever Leave War says it is — the PH flag on the day, or an event word tagged <b>PH</b> (the seeded <b>PH</b> type). An <b>Off day</b> declared by management (the <b>Off day</b> event tag, grey down the column) earns nothing — it is given, not worked. An SC <b>spare</b> earns nothing (standing by at home), and <b>AVALON / BB</b> earn nothing at all, desks included. A row with no times written earns nothing either — the credit follows the timings as scheduled, and a guess would mint OIL from nothing. One start-to-finish measure per day means an hour can never pay twice and a day can never pay more than 1.0. An <b>ALL</b> or <b>ALL AVAIL</b> puck on a Common-Programme or Ground row credits everyone available for that event (aircrew, SANS excluded). A <b>Duty & commitments input</b> is never credited silently: the app asks its owner first, and the confirmed days count into the same start-to-finish day. The credit lands in Leave War as an FO (full) or HO (half) cell and moves the OIL balance there; if the schedule changes and the day is re-issued, the credit follows. It never overwrites a leave bid — that disagreement is raised on Leave War's clash strip for a human to resolve. Every credit — an earned day or one an admin gives — shows in Leave War's <b>OIL tracker</b> with its date and reason; a day of OIL taken spends the <b>oldest</b> credit first, and a credit lasts as long as the admin's OIL policy says (a number of days or months from its own date, or forever) before what is left of it expires.</span>`},
    /* the blind-desk warning (owner, 20 Sep 26) — the rule above says a row
    ...
    {sev:'adv',code:'FLT_NO_LEN',
     t:()=>`An ordinary <b>flying line</b> typed with the <b>same take-off and landing time</b> still earns the crew their OIL; a <b>standby shift</b> (SC, AVALON, BB) typed the same way earns nobody anything at all. Either way one of the two times is wrong and the day says so, on the line, on <b>any</b> day.<span class="why">The sortie itself measures nothing, so on the face of it nobody should be paid for it. They are, and deliberately: a flying day runs from the report three hours before take-off to the debrief two hours after landing, and the crew did both. They were in squadron whatever the times say, which is the squadron's own fact rather than anything the times can show. So the line is never refused and no credit is taken away — the warning exists because a nought-minute sortie is always a slip of the keyboard, and it is cheaper to fix while the day is being built than after it has gone out. It names the line, not the crew: the times are the scheduler's to correct. A line that has no readable times at all is a different problem and carries the other warning — that one really does earn nobody anything. <b>A standby SHIFT is the other way round</b>: SC, AVALON and BB have no report and no debrief, so their window is the written window and one typed to the same minute earns nobody anything at all. The warning says so in those words on a shift, and the day names it — as a shift, not as a desk — in the same list as a desk with no usable times.</span>`},
    {sev:'set',src:()=>`canSpare · sparePost`,
     t:()=>`<b>Local keeps the man reachable</b>, so he may still be raised as an <b>SC SPARE</b> — standby is not a task, and it raises no flag. <b>Overseas cannot</b> (OL, OD), and <b>neither can any of the four medical codes</b>, HL, OML, ATT C and ATT B. Putting one of them on a spare line is a Warning.<span class="why">Standing spare means being reachable and fit to walk. Overseas he is not reachable; medically down he is not fit — which is why the medical codes are barred even though the man is on the island. Besides currency, the pilots-only front seat and not holding another SC seat in the same hours (see The standby lines), this is what IS checked on a spare line.</span>`},
  ...
  {g:'The standby lines',
   ...
   rows:[
    ...
    {sev:'set',src:()=>`scSpare`,
     t:()=>`An <b>SC SPARE</b> is standing by, not tasked: he is checked for <b>currency</b>, for being <b>able to stand it at all</b>, for the <b>front seat</b> — a WSO may not hold a spare front seat — and for <b>not holding another SC seat, or the SC desk, in the same hours</b>. Nothing else: he may fly, sit a sim or stand an ordinary duty in the same hours and none of it raises anything; but an overseas leave or a downchit across the shift does, and so does another SC seat — MAIN or SPARE — or an SC duty desk whose hours overlap his.<span class="why">Overlap means genuinely shared minutes: SC AM 07:00–13:00 into SC PM 13:00–19:00 touch only at 13:00 and stay two clean shifts a man may hold back to back (owner, 31 Aug 26). The SC desk is the duty block whose template is marked <b>SC</b> (owner, 7 Sep 26); a MAIN on that desk is the ordinary conflict. The crew picker refuses the second seat with the same rule the warning uses, so the two never disagree. The rear spare seat carries no seat rule — offered and declined the same day.</span>`},
    {sev:'set',src:()=>`w.noconf`,
     t:()=>`<b>AVALON</b> and <b>BB</b> are exempt from every cross-check against the day's tasks, rest and briefs, and carry four rules of their own — the two are twins (owner, 7 Sep 26), BB's only difference being that its hours are typed rather than fixed, and a BB line left with blank hours is checked for nothing. Every man on it, jet seat or desk, must be <b>on the island</b> and <b>not medically down</b> — overseas leave, overseas duty, hospitalisation, medical leave or ATT C anywhere on the line, and ATT B on a jet seat too, though he may still man the desk. Every seat, MAIN or SPARE, needs <b>SC currency</b> for its hours — SC NIGHT on AVALON's 19:00–07:00, SC DAY on a BB shift typed inside the day. The <b>front seat is pilots-only</b>, MAIN or SPARE — a WSO there is an illegal seat. And <b>nobody holds a seat and another place in the same hours</b> — MAIN and SPARE, a seat and the desk, an AVALON seat and a BB seat; two desk roles on one man are allowed. The AVALON or BB desk is the duty block whose template is marked <b>AVALON</b> or <b>BB</b>; it earns no OIL on a weekend or holiday, and neither does a seat.<span class="why">Local leave, an off in lieu, a course or an appointment plants him with nothing raised — the point of the rule is that standing AVALON is not the same as being cross-checked against the day's tasks (owner, 11 Aug 26; the three seat rules 7 Sep 26). And the shift runs 19:00–07:00, so its morning half is judged against <b>tomorrow's</b> inputs, the same way a night sortie landing after midnight is. A man on AVALON tonight and a sortie tomorrow morning is not a rule — nothing on AVALON is an event.</span>`},
