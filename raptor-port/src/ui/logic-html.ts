  ...
  {g:'Leave, downchit and personal inputs',
   rows:[
    ...
    {sev:'note',set:['oilFullMin'],src:()=>`VCONF.oilFullMin ${VCONF.oilFullMin} · engine/oil.ts`,
     t:()=>`<b>Work on a weekend or public holiday earns OIL</b>, credited automatically into Leave War when the day is <b>published</b>. ONE rule for everything: pool each person's worked hours for the day — flying (report to landing plus debrief), sims, duty rows, the Ground and Common Programme, SC MAIN shifts — and <b>${lgV(ruleFmt('oilFullMin',VCONF.oilFullMin))}</b> or more is a FULL day (FO, 1.0), under it a HALF (HO, 0.5), regardless of the time of day or the shift's shape.<span class="why">A public holiday is whatever Leave War says it is — the PH flag on the day, or an event word tagged "off day" (the seeded <b>PH</b> type). An SC <b>spare</b> earns nothing (standing by at home), and <b>AVALON / BB</b> earn nothing at all, desks included. A row with no times written earns nothing either — the credit follows the timings as scheduled, and a guess would mint OIL from nothing. Overlapping rows count once: the pooled hours are a union, so a man on a flying seat and its ground brief has not worked the hour twice, and a day can never pay more than 1.0. An <b>ALL</b> or <b>ALL AVAIL</b> puck on a Common-Programme or Ground row credits everyone available for that event (aircrew, SANS excluded). A <b>Duty & commitments input</b> is never credited silently: the app asks its owner first, and the confirmed days join the same per-day pool. The credit lands in Leave War as an FO (full) or HO (half) cell and moves the OIL balance there; if the schedule changes and the day is re-issued, the credit follows. It never overwrites a leave bid — that disagreement is raised on Leave War's clash strip for a human to resolve.</span>`},
