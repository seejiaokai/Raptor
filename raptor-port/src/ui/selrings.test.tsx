...
describe('green eligibility rings (13 Aug 26)', () => {
  it('the DOM agrees with slotBar on every edit-week slot', async () => {
    const id = 'wolf'                       // a WSO — front seats must never ring
    await select(id)
  ...
  it('a mutation re-rings on the next paint: an emptied seat brightens', async () => {
    const id = 'wolf'
    await select(id)
    const filled = $$('#eWeek .seat[data-slot].oktake-f').find(el => el.dataset.slot!.endsWith('.w'))
    expect(filled, 'the seed week has a takeable filled rear seat').toBeTruthy()
