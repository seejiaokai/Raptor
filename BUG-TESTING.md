...
| **#368** | Leave War: the on-grid Rearrange bar (Auto-sort / Done) now paints at once on iPhone instead of waiting for a touch — its own compositor layer, one CSS line · the ARCHIVE bar under the manning counters: in Rearrange the eye archives a row out of view, the bar opens to bring one back with ↺ | ⬜ — owner to confirm on his iPhone against the preview (the paint fix is not reproducible here: no WebKit) |
