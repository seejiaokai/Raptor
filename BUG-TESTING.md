...
| **#371** | Leave War: the swipe on the grid behind an OPEN sheet (a cell's bid sheet, the counter picker, a person's figures…) now glides on and slows down with the phone's own momentum instead of stopping dead the moment the finger lifts — the invisible backdrop under the sheet is itself a scroller, mirrored onto the grid, so the swipe IS the phone's native one; a tap on the backdrop still closes the sheet; on a desktop the mouse drag and wheel on the backdrop are unchanged; the sheet opens with the backdrop already lined up to the grid (no jump back to January) | ⬜ — owner to confirm on his iPhone against the preview: open a cell's sheet mid-year, swipe left/right on the grid and let go — it should coast and slow exactly like the grid does with no sheet up; the sheet stays up; a tap on the grid closes it; the month strip and the frozen date bar keep following; swiping up/down still scrolls the page; the grid is where it was when the sheet opened (no jump to JAN) |
