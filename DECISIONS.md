...
| D21 | *"1 nope."* — answering whether a day tagged **"Off day"** should earn OIL | **AN "OFF DAY" DOES NOT EARN OIL. Settled; do not reopen.** It is time GIVEN to a man, not a public holiday he was called in on, so working it earns nothing — only "PH" turns a day into one that can earn. This closes hand-pass finding 16, which both reviewers had already read as a stale sentence in a written contract rather than a bug: **no code changes, the contract wording does.** It also means the Leave War's own second event type stays deliberately inert for OIL, and leave charging is untouched | `raptor-port/docs/ui-contracts.md` (the stale sentence, corrected); `docs/superpowers/specs/2026-09-21-oil-behaviour-register.md`; evidence `docs/handpass/parts/blocks-g.md` finding 16 |
