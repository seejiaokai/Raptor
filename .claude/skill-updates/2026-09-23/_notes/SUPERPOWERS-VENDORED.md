...
3. **Local lessons from the skill review of 23 Sep 26** (owner-approved;
   ...
   |---|---|
   | `verification-before-completion` | seven rows in Common Failures; the "long check runs" pattern; new `browser-checks.md` |
   ...
   | `writing-plans` | plan-file check, per-task browser check, `Model:` line, standing orders, three No-Placeholders items, self-review checks 4-9 |
   | `subagent-driven-development` | pre-flight items, brief contents, machine rules, safe parallelism, DONE_WITH_CONCERNS, decisions-vs-premises, what rides a fix round, ledger/turn-boundary/stop lines, pre-final-review deviation list; `implementer-prompt.md`, `task-reviewer-prompt.md`; `scripts/task-brief` prepends the plan's shared header |
   | `dispatching-parallel-agents` | three Common Mistakes pairs; "When Agents Share the Working Tree or the Machine" |
   | `brainstorming` | visual-first layout rounds; enumerate "every"; layered-rule decision table; trap hunt; the mock-on-the-real-app fallback |
   ...
   | `executing-plans` | premise check before building; deviation list before review |
   | `using-git-worktrees` | the symlinked-dependencies staging trap |
