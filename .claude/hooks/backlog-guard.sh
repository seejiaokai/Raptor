...
{
  echo "BACKLOG GUARD: a filed record in OUTSTANDING.md / OUTSTANDING-ARCHIVE.md looks lost or damaged."
  echo "$out" | grep -E '^\s+- |^FAIL' 
  echo "Restore it from the base before stopping. If it is deliberate, declare it in the commit (Docs-guard-allow:)."
} >&2
exit 2
