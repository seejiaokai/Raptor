...
|---|---|---|---|---|
| D191 | 25 Sep 26 | **"A"** — asked what OK should do on "+ Add" when a callsign NOT on the roster has been typed into the roster SEARCH (the cursor starts there) and the "Or type a callsign" box is empty: (A, recommended) add that name as a new crew member, the line under the search saying so before OK is pressed; (B) refuse and point to the box below; (C) leave it | **OK ADDS IT.** When the search matches nobody on the roster and "Or type a callsign" is empty, OK adds what was typed in the search as a new, unlinked crew member — exactly as if it had been typed in the box below — and the "Nobody on the roster matches" line says so while it is true, before OK is pressed. **Unchanged:** a name typed in the box below always wins; a search that still matches someone adds nothing by itself (pick the person, or type in the box); with no roster (the standalone Tracker) the box is the old prompt. **The agent's call, stated to him:** Enter in the search does the same as OK in that case — the keyboard's door to the same action — as it already picks a sole match | `OUTSTANDING.md` `[TRK-ADD-SEARCH-OK]`; on build: `raptor-port/src/tracker/components/Modals.jsx` (`DlgModal`), `raptor-port/docs/ui-contracts.md` §The Tracker tab (the + Add entry) |
