...
no OIL). **Also read** — in `oil.md`: **D260** (a dragged block's Delete and the one-day Clear remove OIL awards too, and name them first; awards never move). In `how-we-work.md`: **D166** (accounts: the war follows the signed-in callsign, not "View as"). In `scheduler.md`: **D44, D45** (nothing on a published day changes without the
...
|---|---|---|---|---|
| D262 | 27 Sep 26 | *"i want to be able to click on a single chip on the leave war and the move button should be enabled for me to click to move the chip. The calendar can be removed. Take note with the move enabled when i drag to the edges of the leave war it should auto scroll, or i still have the option to click on other months and the move function will still be enabled. unless i click on an empty area outside the leave war grids to cancel that function."* — said while answering the absence-record re-test's questions | **ONE CHIP, ONE MOVE: THE ONE-DAY SHEET'S MOVE IS ALWAYS PRESSABLE AND PICKS THE CHIP UP — NO DATE BOX.** Tap a single chip → its sheet → **Move** (never greyed out) → the chip is in move mode and he lands it on a day of the grid. The date box beside Move ("the calendar") **goes**. While moving: (1) **dragging to the grid's edges scrolls it** (left / right, and up / down where the grid scrolls); (2) **the month buttons still work and move mode stays on**; (3) **a click on an empty area OUTSIDE the grid cancels** the move. **The agent's readings, stated to him so he can correct them:** (a) it covers both one-day sheets that carry a Move today (the bid sheet's decision row and the decide sheet); (b) a landing keeps today's rules — refused whole with its reason on a day that cannot take it, the dotted "moved" mark once bidding is closed, it lands undecided for the admin to approve (a move is a proposal); (c) the desktop lands on the click or the release; the phone keeps its tap-then-Confirm two-step; (d) the drag-selection's own "Move…" gets the same three behaviours (edge scroll, months keep it on, a click outside cancels), so a single chip and a block move alike; (e) a stage or war change, and Undo, still end a move (the existing guard) | **BUILT 27 Sep 26** on the branch claude/absence-record-d147-af6a50 (PR #444, before his "merge live"): `raptor-port/src/leavewar/ui/BidPicker.tsx` (Move → `onMove`, the date box and the unmounted `DecisionSheet` removed), `raptor-port/src/leavewar/ui/Matrix.tsx` (the one-cell `moveSel`, `inGrid` / `moveLeftEdge`, the own-day words, a move ended when the war leaves the screen), `raptor-port/src/leavewar/ui/select.ts` (`wireMove`: the mouse's edge scroll, the press-and-drag, the outside-tap cancel, the double-click guard, the mouse-only right-click); pinned by `moveone.test.tsx`, `movewire.test.ts`, `deciding.test.tsx`, e2e `moveOneTo`; `raptor-port/docs/ui-contracts.md` §Selecting on the Leave War grid; evidence `raptor-port/docs/handpass/2026-09-27-d260-d262.md` (C1–C12); `OUTSTANDING-ARCHIVE.md` `[LW-MOVE-ONE-CHIP]`. **Left for him:** the tap list's per-record Move… keeps its date box (reading (a)) — `OUTSTANDING.md` `[LW-MOVE-TAPLIST-ASK]` |
