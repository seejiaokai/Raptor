...
### Observation 162: The claudex-loop runner exits 1 on a Windows console-encoding error AFTER a fully successful review — read result.json, never trust the exit code alone

**Status:** OPEN
**Date:** 2026-09-17
**Session context:** [CMDL-FINISH] design red-team — ran the claudex-loop runner (`review --host claude --model gpt-6-astra`) on Windows across four rounds. Three separate completed reviews returned a background exit code of 1.

**Issue:** The runner completed the Codex review and wrote a valid `result.json` (verdict + findings intact), then CRASHED while pretty-printing the result to the console: `'charmap' codec can't encode character '→'` (a Unicode arrow, cp1252). The process exits 1. A coordinator watching only the task-completion exit code would read this as a FAILED review and either retry (burning a review + provider quota) or, worse, treat it as a blocker. Distinct from a genuine early failure (e.g. a resume that hit "model at capacity" exited in 3–21s with an `error` field and no `response`) — the encoding crash happens only AFTER a full ~5-min run with a complete `response`. The pattern recurred every round whose findings contained an arrow (`→`), which review findings frequently do.
